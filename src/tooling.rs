...
#[cfg(feature = "syntax")]
pub use lyma_syntax::{TextEdit, TextRange, apply_text_edits};

#[cfg(feature = "parser")]
pub use lyma_parser::{FormatRangeError, FormatRangeFallback, FormatRangeOptions};

/// Formats a whole source buffer into a single replacement edit.
#[cfg(feature = "parser")]
#[must_use]
pub fn format_document_edit(name: &str, source: &str) -> lyma_parser::ParsedFormatting {
    lyma_parser::format_str(lyma_parser::FileId(1), name, source)
}
...
pub fn format_document_text_edit(
    name: &str,
    source: &str,
) -> (lyma_parser::ParsedFormatting, TextEdit) {
...
pub fn format_document_text_edits(
    name: &str,
    source: &str,
) -> (lyma_parser::ParsedFormatting, Vec<TextEdit>) {
    let formatted = format_document_edit(name, source);
...
) -> Result<(lyma_parser::ParsedFormatting, Vec<TextEdit>), FormatRangeError> {
    lyma_parser::format_range_edits(lyma_parser::FileId(1), name, source, range, options)
}

/// Serializes an evaluated portable value into canonical Lyma text.
///
/// This remains the stable tooling entry point for callers that already work
/// with `lyma_syntax::LymaValue` directly. For typed Rust/Serde data models,
/// prefer `lyma_serde` or the facade helpers under `lyma::serde` to produce a
/// portable value first and then serialize it canonically.
#[cfg(feature = "syntax")]
pub fn serialize_portable_value(
    value: &lyma_syntax::LymaValue,
) -> Result<String, lyma_syntax::Diagnostic> {
    lyma_syntax::serialize_value(value)
}
