# LUMBA Binary Format Specification

Status: draft 0.1  
Format name: LUMBA  
Related text format: LUMA, Lua Unified Model Assembly  
Recommended file extension: `.lumba`  
Recommended media type: `application/vnd.lumba`  
Default byte order: little-endian  
Execution model: parse-only and inert by default

LUMBA is the binary companion format for LUMA. It stores LUMA documents, evaluated LUMA value graphs, optional syntax trees, optional source maps, optional schemas, and optional bundles in a compact, deterministic, random-access container.

LUMBA is not a Lua bytecode format. LUMBA parsers must not execute Lua. Lua expressions, Lua chunks, host module references, import requests, and other executable LUMA constructs are encoded only as inert data unless a host application explicitly evaluates them under a separate capability policy.

---

## 1. Purpose

LUMA is optimized for people. It is text-first, diffable, editable, and scriptable. LUMBA is optimized for machines. It is binary-first, compact, indexed, deterministic, and suitable for fast loading.

A host may use LUMBA for:
...
The same logical LUMA document may have several valid LUMBA representations depending on what is preserved:
...
2. Safe by default
   - opening a LUMBA file never executes Lua;
...
5. LUMA-aware
   - LUMA null is preserved distinctly from absent fields;
...
- a mandatory runtime format for every LUMA implementation.
...
## 4. Relationship to LUMA

LUMBA is a representation of the LUMA data model, not a different language.

A LUMA text document may be converted to LUMBA in one of three major ways:

```text
LUMA text -> parse -> LUMBA syntax image
LUMA text -> parse -> evaluate -> LUMBA value image
LUMA text plus imports -> assemble -> LUMBA bundle image
```
...
Both are valid LUMBA use cases.

---

## 5. Conformance levels

A LUMBA implementation should declare the highest level it supports.
...
| Document | One logical LUMA document in a document stream. |
| Syntax node | One node in the parsed LUMA source tree. |
...
| Canonical LUMBA | A deterministic byte representation with restricted options. |

---

## 7. High-level file layout

A LUMBA file has this structure:
...
The magic bytes are ASCII `LUMBA`, followed by carriage return, line feed, and substitute. The substitute byte helps text-mode transfer damage show up early.
...
| --- | --- | --- | --- |
| `META` | Metadata | Level 0 | Container metadata as LUMBA values. |
...
| `ASTN` | Syntax nodes | Level 3 | Parsed LUMA syntax tree arena. |
...
| `EMBD` | Embedded resources | Level 4 | Embedded LUMA or LUMBA resources. |
...
A minimal value-only LUMBA file needs `STRS`, `VALS`, and `DOCS`. `SYMS` is recommended but may be omitted if no symbol-specific records exist.
...
- Encodings must be minimal in canonical LUMBA.
...
Portable canonical LUMBA must reject NaN and infinity.
...
Absent is not the same as LUMA null. LUMA null is an explicit value record.
...
UVar luma_version_string_ref_or_absent
UVar lumba_version_string_ref_or_absent
UVar reserved_count
...
```text
org.luma.compression.zstd
org.luma.signature.ed25519
org.luma.lua.bytecode.lua54
org.luma.schema.inline
```

Extension names use reverse-DNS or `org.luma.*` names. Private extensions should not use the `org.luma` namespace.
...
Canonical LUMBA should intern strings by first occurrence in deterministic traversal order.
...
The namespace field is absent when zero. When present, it references a string such as `luma`, `schema`, `syntax`, or an extension namespace.
...
LUMA null is encoded as tag `0x00`. It is not equivalent to absent references.
...
Canonical LUMBA should use `BytesBlob` for byte arrays larger than 64 bytes.
...
Map order is preserved. Duplicate keys are invalid in canonical LUMBA.

Core portable maps should use string keys. LUMBA can encode non-string keys because evaluated Lua tables may contain them, but schema-bound and canonical public interchange should reject non-string keys unless the schema explicitly permits them.
...
Duplicate key detection uses LUMA equality rules for keys. For canonical portable maps, keys are UTF-8 strings and equality is byte-for-byte equality.
...
Core LUMBA stores Lua source, not Lua bytecode.
...
The `DOCS` section stores the logical LUMA document stream.
...
Example tag registry as LUMA metadata:

```luma
tags:
  - name: Service
    uri: urn:luma:example:service
    portable: true
  - name: Duration
    uri: urn:luma:example:duration
    portable: true
```
...
A LUMBA file may contain diagnostics and still be loadable. A host policy decides whether warning-bearing or error-bearing files are accepted.
...
The `ASTN` section stores parsed LUMA syntax nodes.
...
| Value | Name | Meaning |
| ---: | --- | --- |
| 0 | import | LUMA `@import` |
| 1 | include | LUMA `@include` |
| 2 | module | LUMA `@use` host module |
| 3 | schema | schema reference |
| 4 | source | original source file |
| 5 | generated | generated intermediate |
| 6 | external_resource | non-LUMA external resource |
...
Core LUMBA does not mandate any signature algorithm. Signature verification is a host policy.
...
Core LUMBA requires support only for uncompressed sections.
...
Canonical LUMBA is a restricted encoding intended for reproducible builds, content hashing, cache keys, and conformance tests.
...
LUMBA preserves LUMA value equality rules.
...
When a map contains non-string keys, canonical ordering is not required by core LUMBA because source order is preserved. If a host chooses to sort non-string keys, it must document the policy.

---

## 35. Lua and executable content

LUMBA treats all executable material as inert data.

Core rules:

- LUMBA loading must not execute Lua.
- LUMBA loading must not compile Lua.
- Lua expressions are stored as source text or as syntax nodes.
- Lua chunks are stored as source text or blobs.
- Evaluation results may be stored as normal LUMBA values.
...
Public LUMBA interchange should not rely on bytecode.

---

## 36. Security model

A LUMBA reader must defend against malicious binary input.
...
LUMBA readers should report structured diagnostics with:
...
A value-only LUMBA file would contain:
...
schemas/service.schema.luma
```

The LUMBA bundle would contain:
...
LUMBA to LUMA text conversion depends on what was preserved.

| LUMBA contents | Possible text output |
| --- | --- |
| value only | canonical LUMA text with no comments or original expressions |
| value plus spans | canonical LUMA text with diagnostics mapped to source |
| syntax plus trivia | mostly lossless source reconstruction |
| syntax without trivia | structured pretty-printed LUMA |
...
## 44. Interoperability with LUMA constructs

| LUMA construct | Value image | Syntax image |
| --- | --- | --- |
...
```text
com.example.lumba.experimental.feature_name
```

They should not use `org.luma` unless accepted into the standard namespace.
...
LUMBA is the binary representation of LUMA, Lua Unified Model Assembly. It provides a compact, deterministic, sectioned container for LUMA values, document streams, syntax trees, source maps, schemas, dependencies, diagnostics, and optional bundle metadata.

The core principles are:

```text
load bytes safely
preserve LUMA semantics
...
A simple LUMBA file can be just strings, values, and documents. A rich LUMBA file can preserve the full parsed source structure and dependency graph. Both remain part of the same format because both represent the same LUMA model at different stages of the parse, assembly, and evaluation pipeline.
