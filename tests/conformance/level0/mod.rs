...
use lyma_syntax::{DiagnosticCode, FileId, LymaNode, MappingItem, SequenceItem, StringStyle};
...
fn level0_parses_static_data_scalars_and_blocks() {
    ...
    let LymaNode::Mapping(root) = root(&parsed) else {
        panic!()
    };
    ...
    let LymaNode::String(title) = &entry.value else {
        panic!()
    };
    ...
    let LymaNode::String(description) = &entry.value else {
        panic!()
    };
    ...
    let LymaNode::Sequence(items) = &entry.value else {
        panic!()
    };
    assert_eq!(items.items.len(), 3);
    assert!(matches!(
        items.items[1],
        SequenceItem::Value(LymaNode::Null { .. })
    ));
}
...
fn level0_rejects_invalid_inputs() {
    for (source, code) in [
        ...
        let parsed = parse_str(FileId(1), "invalid.lyma", source);
        assert!(
...
fn level0_preserves_mapping_and_sequence_order() {
    ...
    let LymaNode::Mapping(inner) = &entry.value else {
        panic!()
    };
    ...
    let LymaNode::Sequence(list) = &entry.value else {
        panic!()
    };
    assert_eq!(list.items.len(), 3);
}

fn root(parsed: &lyma_parser::Parsed) -> LymaNode {
    let document = &parsed.file.documents[0];
    let lyma_syntax::DocumentItem::Root(root) = &document.items[0] else {
        panic!()
    };
    root.clone()
}
    ...
    fn entry_key(&self) -> &str {
        match self {
            Self::Entry(entry) => match &entry.key {
                lyma_syntax::MappingKey::Plain { value, .. } => value,
                lyma_syntax::MappingKey::Quoted(node) => &node.value,
                lyma_syntax::MappingKey::Expression { .. } => panic!(),
            },
            _ => panic!(),
        }
    }
