...
use lyma::lyba::container::{ContainerHeader, HeaderCrcMode, SECTION_ENTRY_SIZE};
use lyma::lyba::primitives::Identifier;
use lyma::lyba::section::SectionEntry;
...
use lyma::lyba::{
    ...
    EmbeddedResourceTable, Limits, LybaError, LybaFile, MapEntry, ReadOptions, Reader,
...
pub fn manifest_text(relative: &str) -> String {
    fs::read_to_string(fixture_root().join(relative))
        .unwrap_or_else(|_| panic!("missing LYBA fixture manifest {relative}"))
}

pub fn checked_in_level1_fixture() -> &'static [u8] {
    include_bytes!("../../fixtures/lyba/level1/minimal-values.lyba")
}
...
pub fn level0_negative_error() -> LybaError {
...
pub fn level2_positive_bytes() -> Vec<u8> {
    canonical_writer()
        .write(
            &LybaFile::new()
                .with_document(
                    Document::new().with_root_value(Value::Tagged(lyba::TaggedValue {
...
pub fn level2_negative_error() -> LybaError {
    let bytes = canonical_writer()
        .write(
            &LybaFile::new().with_tag_table(
                TagTable::new().with_declaration(
                    TagDeclaration::new("Secret", "urn:lyma:example:secret")
...
pub fn level3_negative_error() -> LybaError {
    let mut blobs = BlobTable::new();
    blobs
        ...
        .write(
            &LybaFile::new()
                ...
                .with_source_file_table(
                    SourceFileTable::new().with_record(
                        SourceFileRecord::new()
                            .with_uri(Some(String::from("mem://private.lyma")))
                            .with_source_blob_ref(Some(BlobId(0)))
            ...
            &LybaFile::new()
                ...
                .with_dependency_table(
                    DependencyTable::new().with_record(
                        DependencyRecord::new(lyba::DEPENDENCY_KIND_EXTERNAL_RESOURCE)
                            .with_uri(Some(String::from("https://127.0.0.1:9/never-contacted")))
                            .with_flags(
                                lyba::DEPENDENCY_FLAG_EMBEDDED
                                    | lyba::DEPENDENCY_FLAG_NETWORK_URI,
                            ),
                    ),
                )
                .with_embedded_resource_table(EmbeddedResourceTable::new().with_record(
                    EmbeddedResourceRecord::new(
                        0,
                        lyba::EMBEDDED_RESOURCE_KIND_LYMA_TEXT,
                        BlobId(0),
                    ),
                ))
...
pub fn level4_negative_error() -> LybaError {
    Writer::new(WriteOptions::new())
        .write(
            &LybaFile::new()
                .with_dependency_table(
                    ...
                    Document::new()
                        .with_root_value(Value::Map(vec![
                            MapEntry {
                                ...
                                value: Value::ExpressionSource(lyba::ExpressionValue {
                                    language: Identifier::new("lua.expr"),
                                    source: lyba::ExpressionSource::Text(String::from("1 + 1")),
                            ...
                            MapEntry {
                                ...
                                value: Value::LuaChunkSource(lyba::LuaChunkValue {
                ...
                .with_capability_table(CapabilityTable::new().with_record(
                    CapabilitySetRecord::new().with_requirement(
                        CapabilityRequirement::new(Identifier::new("lua.eval.expr")).with_flags(
                            lyba::CAPABILITY_FLAG_REQUIRED_FOR_EVALUATION
                                | lyba::CAPABILITY_FLAG_PURE_EXPECTED
                                | lyba::CAPABILITY_FLAG_DETERMINISTIC_EXPECTED,
                        ),
                    ),
                )),
        )
...
pub fn level5_negative_error() -> LybaError {
    let bytes = canonical_writer()
        .write(
            &LybaFile::new().with_capability_table(
                CapabilityTable::new().with_record(
                    CapabilitySetRecord::new().with_requirement(
                        CapabilityRequirement::new(Identifier::new("module.resolve"))
                            .with_flags(lyba::CAPABILITY_FLAG_TRUSTED_ONLY),
                    ),
                ),
            ),
        )
...
pub fn canonical_negative_error() -> LybaError {
    let bytes = Writer::new(WriteOptions::new())
        .write(&LybaFile::new().with_document(Document::new().with_root_value(Value::Int(7))))
...
fn canonical_writer() -> Writer {
    Writer::new(
        WriteOptions::new()
            .with_mode(lyba::WriterMode::Canonical(lyba::CanonicalMode::Strict))
