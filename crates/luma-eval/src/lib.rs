...
use luma_runtime::{Engine, LuaRuntimeEngine, LuaRuntimeError, LuaSourceText, RuntimeLimits};
...
pub mod evaluate {
    use luma_runtime::{LuaRuntimeEngine, LuaRuntimeError, LuaSourceText, RuntimeLimits};
...
impl<E: LuaRuntimeEngine> EvaluationPlan<E> {
    ...
    pub fn evaluate_expression(
        ...
        source: LuaSourceText<'_>,
        limits: &RuntimeLimits,
    ) -> Result<<E as luma_runtime::RuntimeValueCodec>::Value, LuaRuntimeError> {
...
mod tests {
    ...
    impl RuntimeEnvironmentFactory for MockEngine {
        ...
        fn create_environment(&self) -> Result<Self::Environment, LuaRuntimeError> {
            ...
            environment.inject_builtin(
                "answer",
                MockValue(LumaValue::Number(luma_syntax::LumaNumber::Integer(42))),
            )?;
            environment
                .inject_context("name", MockValue(LumaValue::String(String::from("luma"))))?;
            environment.inject_module(MockModule {
                name: String::from("safe"),
                exports: vec![(String::from("enabled"), MockValue(LumaValue::Boolean(true)))],
            })?;
            Ok(environment)
        }
    }
        ...
        fn compile_expression(
            ...
            if source.text.contains("syntax_error") {
                return Err(LuaRuntimeError::syntax_error(
                    self.engine_name(),
                    LuaRuntimePhase::Compile(luma_runtime::LuaChunkKind::Expression),
                    "mock syntax error",
                    source.span,
                ));
            }
    ...
    fn evaluation_plan_compiles_and_runs_against_mock_engine() {
        ...
        assert_eq!(
            expression.0,
            LumaValue::Number(luma_syntax::LumaNumber::Integer(42))
        );
        assert_eq!(chunk.0, LumaValue::String(String::from("luma")));
    }
    ...
    fn mock_engine_reports_backend_neutral_diagnostics() {
        let plan = EvaluationPlan { engine: MockEngine };
        let limits = RuntimeLimits::sandboxed();
        let span = Span::new(luma_syntax::FileId(7), 1, 13);
