...
use luma_syntax::{Diagnostic, DiagnosticCode, Severity};
...
mod tests {
    ...
    fn in_memory_resolver_rejects_cycles_and_depth() {
        ...
        assert_eq!(
            ...
            luma_syntax::DiagnosticCode::ResourceLimitExceeded
        );
        ...
        assert_eq!(
            ...
            luma_syntax::DiagnosticCode::ImportCycle
        );
    }
    ...
    fn filesystem_resolver_rejects_symlink_escapes() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir().join(format!("luma-eval-{unique}"));
        let outside = std::env::temp_dir().join(format!("luma-eval-outside-{unique}"));
