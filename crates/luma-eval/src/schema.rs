...
pub struct InMemorySchemaValidator {
    max_depth: usize,
    schemas: BTreeMap<String, LumaValue>,
}
...
impl InMemorySchemaValidator {
    ...
    #[must_use]
    pub fn with_schema(mut self, specifier: impl Into<String>, expected: LumaValue) -> Self {
...
mod tests {
    ...
    fn deny_all_schema_validator_rejects_by_default() {
        ...
        let error = DenyAllSchemaValidator
            .validate(SchemaValidationRequest {
                schema: "schemas/app",
                value: &luma_syntax::LumaValue::Boolean(true),
                from: None,
                context: &mut context,
            })
    ...
    fn in_memory_schema_validator_uses_stable_resolution_rules() {
        let validator = InMemorySchemaValidator::new(4)
            .with_schema("schemas/flag", luma_syntax::LumaValue::Boolean(true));
