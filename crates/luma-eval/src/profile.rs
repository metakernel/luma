...
use luma_runtime::RuntimeLimits;
use luma_syntax::{
    Diagnostic, DiagnosticCode, LumaKey, LumaMapping, LumaSequence, LumaTaggedValue, LumaValue,
    Severity,
};
...
pub trait ProfilePolicy {
    ...
    fn validate_runtime_output(&self, value: &LumaValue) -> Result<(), ProfileError> {
...
fn validate_value(profile: &EvaluationProfile, value: &LumaValue) -> Result<(), ProfileError> {
    match value {
        LumaValue::Null(_)
        | LumaValue::Boolean(_)
        | LumaValue::Number(_)
        | LumaValue::String(_) => Ok(()),
        LumaValue::Sequence(LumaSequence { items, .. }) => items
            .iter()
        ...
        LumaValue::Tagged(LumaTaggedValue { value, .. }) => validate_value(profile, value),
        LumaValue::Function(_) if profile.runtime_output.allow_function_values => Ok(()),
        LumaValue::Function(_) => Err(ProfileError::new(
            DiagnosticCode::FunctionValueNotAllowedInThisProfile,
            format!("profile '{}' forbids function values", profile.name),
        )),
        LumaValue::UserData(_) if profile.runtime_output.allow_user_data => Ok(()),
        LumaValue::UserData(_) => Err(ProfileError::new(
            DiagnosticCode::UnsupportedProfile,
            format!("profile '{}' forbids userdata values", profile.name),
        )),
        LumaValue::HostObject(_) if profile.runtime_output.allow_host_objects => Ok(()),
        LumaValue::HostObject(_) => Err(ProfileError::new(
            DiagnosticCode::UnsupportedProfile,
...
fn validate_key(profile: &EvaluationProfile, key: &LumaKey) -> Result<(), ProfileError> {
    match key {
        LumaKey::String(_) | LumaKey::Number(_) | LumaKey::Boolean(_) => Ok(()),
        LumaKey::Host(_)
        ...
        LumaKey::Host(_) => Err(ProfileError::new(
...
#[cfg(test)]
mod tests {
    use luma_syntax::{LumaHostValue, LumaMapping, LumaMappingEntry, LumaNull};
    ...
    fn restricted_profile_denies_runtime_host_values() {
        ...
        let error = profile
            .validate_runtime_output(&luma_syntax::LumaValue::Function(LumaHostValue {
    ...
    fn deterministic_mode_rejects_host_mapping_keys() {
        let profile = EvaluationProfile {
            ...
            runtime_limits: luma_runtime::RuntimeLimits::sandboxed(),
        };
        let error = profile
            .validate_runtime_output(&luma_syntax::LumaValue::Mapping(LumaMapping {
                entries: vec![LumaMappingEntry {
                    key: luma_syntax::LumaKey::Host(LumaHostValue {
                        kind: String::from("host"),
                        label: None,
                    }),
                    value: luma_syntax::LumaValue::Null(LumaNull),
                    span: None,
                }],
