...
use luma_runtime::{LuaRuntimeError, RuntimeEnvironment, RuntimeModule};
...
pub struct OmniLuaModule {
    ...
    pub(crate) exports: Vec<(String, LumaValue)>,
}
...
pub struct OmniLuaEnvironment {
    builtins: Vec<(String, LumaValue)>,
    context: Vec<(String, LumaValue)>,
    ...
    pub(crate) fn materialize(&self) -> Result<Lua, LuaRuntimeError> {
        let lua = Lua::try_new().map_err(|error| {
            LuaRuntimeError::runtime_error(
                engine_name(),
                luma_runtime::LuaRuntimePhase::Environment,
