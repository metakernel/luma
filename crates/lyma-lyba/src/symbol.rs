...
impl SymbolInterner {
    ...
    pub fn intern_profile(&mut self, value: impl AsRef<str>) -> Result<u64> {
        self.intern_symbol(value, Some("lyma"), SYMBOL_FLAG_PROFILE)
    }
        ...
        let namespace_encoded = record
            ...
            .map(|value| {
                value.checked_add(1).ok_or_else(|| {
                    LybaError::invalid_section_table("symbol namespace reference overflowed")
                })
            })
            .transpose()?
            .unwrap_or(0);
    ...
    fn interner_is_deterministic_and_merges_flags_per_name_and_namespace() {
        ...
        assert_eq!(
            strings
                ...
                .collect::<Vec<_>>(),
            vec![
                "name", "lyma", "schema", "mapping", "syntax", "safe", "widget", "acme"
            ]
        );
