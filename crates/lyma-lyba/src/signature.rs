...
//!
//! Core LYBA does not define a trusted algorithm set and this module does not
...
pub(crate) fn decode_signature_table(
    ...
    let record_count = usize::try_from(UVar::decode(payload, &mut offset)?.0).map_err(|_| {
        LybaError::limit_exceeded("signature record count exceeds configured maximum")
    })?;
...
pub(crate) fn encoded_section_manifest(file: &LybaFile) -> Vec<SectionId> {
...
fn validate_record_kind(kind: u64, record_index: usize) -> Result<()> {
    if kind <= SIGNATURE_RECORD_KIND_EXTENSION {
        ...
        Err(LybaError::InvalidSectionTable(
...
fn encode_optional_symbol_string_ref(
    ...
    symbol_id.checked_add(1).ok_or_else(|| {
        LybaError::invalid_section_table("signature algorithm symbol reference overflowed u64")
    })
}
...
fn encode_blob_ref(
    ...
    blob_ref
        .0
        .checked_add(1)
        .ok_or_else(|| LybaError::invalid_section_table("signature blob reference overflowed u64"))
}
...
fn encode_value_ref(
    ...
    index.checked_add(1).ok_or_else(|| {
        LybaError::invalid_section_table("signature value reference overflowed u64")
    })
}
