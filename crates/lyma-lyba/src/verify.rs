...
use crate::container::{ContainerHeader, HEADER_SIZE, HeaderCrcMode, LybaFile};
...
pub(crate) fn verify_level1_minimal_value_image_file(file: &LybaFile) -> Result<()> {
    if file.id.is_some() {
        return Err(LybaError::invalid_section_table(
    ...
    if file.documents.iter().any(|document| {
        ...
        return Err(LybaError::InvalidDocumentTable(
...
impl Verifier {
    ...
    pub fn verify(&self, file: &LybaFile) -> Result<VerificationReport> {
...
fn decode_sections_relaxed<'a>(
    ...
    let table_end = header
        ...
        .ok_or_else(|| {
            LybaError::offset_outside_file("section table offset plus length overflowed")
        })?;
    if table_end > bytes.len() as u64 {
        return Err(LybaError::offset_outside_file(
            "section table extends beyond available input",
        ));
    }

    let table_start = usize::try_from(header.section_table_offset).map_err(|_| {
        LybaError::offset_outside_file("section table offset was not representable")
    })?;
    let table_end = usize::try_from(table_end)
        .map_err(|_| LybaError::offset_outside_file("section table end was not representable"))?;
    let table_bytes = bytes
        .get(table_start..table_end)
        .ok_or_else(|| LybaError::offset_outside_file("section table bytes were not available"))?;
    ...
    for entry_bytes in table_bytes.chunks_exact(crate::container::SECTION_ENTRY_SIZE as usize) {
        ...
        if payload_end > bytes.len() as u64 {
            return Err(LybaError::offset_outside_file(format!(
    ...
    for pair in spans.windows(2) {
        if pair[0].1 > pair[1].0 {
            return Err(LybaError::OverlappingSections(ErrorContext::new(
...
fn verify_canonical_strs_payload(section: &ValidatedSection<'_>, limits: &Limits) -> Result<usize> {
    ...
    for _ in 0..string_count {
        ...
        core::str::from_utf8(bytes)
            .map_err(|_| LybaError::invalid_utf8("string bytes were not valid UTF-8"))?;
    }
...
fn verify_canonical_syms_payload(
    ...
    let symbol_count = usize::try_from(symbol_count)
        .map_err(|_| LybaError::limit_exceeded("symbol count exceeds configured maximum"))?;
    ...
    let table =
        decode_symbol_table(section.payload, limits, string_count).map_err(
            |error| match error {
                LybaError::InvalidReservedFlags(_) => {
                    noncanonical("canonical SYMS records require zero reserved flag bits")
                }
                LybaError::InvalidValueReference(_) => error,
                _ => error,
            },
        )?;
...
fn verify_canonical_vals_payload(section: &ValidatedSection<'_>, limits: &Limits) -> Result<usize> {
    ...
    let offset_table_end = offset
        .checked_add(value_count.checked_mul(8).ok_or_else(|| {
            LybaError::invalid_section_table("value offset table length overflowed")
        })?)
        .ok_or_else(|| LybaError::invalid_section_table("value record start overflowed"))?;
    ...
    for _ in 0..value_count {
        ...
        record_offsets.push(usize::try_from(relative).map_err(|_| {
            LybaError::invalid_section_table("value record offset was not representable")
        })?);
    }
    ...
    for record_index in 0..value_count {
        ...
        let record = section
            ...
            .ok_or_else(|| {
                LybaError::invalid_section_table("value record bounds were outside the payload")
            })?;
        if record.is_empty() {
            return Err(LybaError::invalid_section_table(
        ...
        match tag {
            ...
            RECORD_TAGGED => {
                ...
                let bytes =
                    read_bounded_bytes(record, &mut record_offset, len, limits.max_string_bytes)?;
                core::str::from_utf8(bytes)
                    .map_err(|_| LybaError::invalid_utf8("tag bytes were not valid UTF-8"))?;
                let value_ref = decode_canonical_uvar(record, &mut record_offset)?.0;
                if value_ref >= value_count as u64 {
                    return Err(LybaError::InvalidValueReference(ErrorContext::new(
                        "value reference pointed outside the value arena",
            ...
            _ => {
                return Err(LybaError::invalid_section_table(format!(
...
fn verify_zero_gap_bytes_canonical(bytes: &[u8], gap_start: u64, gap_end: u64) -> Result<()> {
    ...
    let start = usize::try_from(gap_start)
        .map_err(|_| LybaError::offset_outside_file("gap start was not representable"))?;
    let end = usize::try_from(gap_end)
        .map_err(|_| LybaError::offset_outside_file("gap end was not representable"))?;
    let gap = bytes.get(start..end).ok_or_else(|| {
        LybaError::offset_outside_file("gap bytes extended beyond available input")
    })?;
...
fn noncanonical(message: impl Into<String>) -> LybaError {
    LybaError::non_canonical_encoding(message)
}

fn diagnostic_to_error(diagnostic: &Diagnostic) -> LybaError {
    let mut context = ErrorContext::new(diagnostic.message.clone());
    context.record_index = diagnostic.record_index;
    match diagnostic.code {
        DiagnosticCode::DuplicateKeyInCanonicalMap => {
            LybaError::DuplicateKeyInCanonicalMap(context)
        }
        _ => LybaError::NonCanonicalEncoding(context),
    }
}
...
mod tests {
    ...
    fn verifier_rejects_duplicate_strings_as_non_canonical_lb0017() {
        let file = LybaFile::new().with_string_table(
            StringTable::new()
    ...
    fn verifier_rejects_duplicate_canonical_map_keys_as_lb0016() {
        let file = LybaFile::new().with_section(Section {
    ...
    fn level1_minimal_verifier_rejects_metadata() {
        let error = super::verify_level1_minimal_value_image_file(&LybaFile::new().with_metadata(
            Metadata::new().with_entry("format", Value::String(String::from("lyba"))),
        ))
