...
use crate::container::{
    ContainerHeader, DocumentImage, HeaderCrcMode, LybaFile, discover_footer,
...
/// Reader entry point for decoding LYBA documents.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct Reader {
    options: ReadOptions,
}
...
impl Reader {
    ...
    pub fn read(&self, bytes: &[u8]) -> Result<LybaFile> {
        ...
        file.documents = if let Some((section_index, section)) = docs_section {
            ...
            let value_roots = file
                ...
                .map(|section| section.values.as_slice())
                .ok_or_else(|| {
                    LybaError::InvalidDocumentTable(crate::error::ErrorContext::new(
        ...
        match value {
            ...
            Value::RuntimeDescriptor(descriptor) => {
                if descriptor.trusted_only && !self.options.limits.allows_trusted_only() {
                    return Err(LybaError::trusted_only_rejected(format!(
                ...
                if descriptor.required && !self.options.limits.allows_trusted_only() {
                    return Err(LybaError::UnsafeEvaluationRequest(
                        crate::error::ErrorContext::new(format!(
    ...
    fn enforce_extension_policy(
        ...
        for declaration in &extension_table.declarations {
            ...
            {
                return Err(LybaError::UnsupportedRequiredExtension(
                    crate::error::ErrorContext::new(format!(
    ...
    fn enforce_schema_policy(&self, schema_table: &crate::schema::SchemaTable) -> Result<()> {
        for record in &schema_table.records {
            if record.requires_trusted_validator() && !self.options.limits.allows_trusted_only() {
                return Err(LybaError::trusted_only_rejected(
                ...
                SectionId::STRS => {
                    total_strings =
                        total_strings.checked_add(entry.item_count).ok_or_else(|| {
                            LybaError::limit_exceeded("string count exceeds configured maximum")
                        })?;
                    enforce_limit_u64(
                        total_strings,
...
pub(crate) fn read_level1_minimal_value_image(bytes: &[u8]) -> Result<Vec<LymaValue>> {
...
fn verify_level1_minimal_value_image_bytes(bytes: &[u8]) -> Result<()> {
    ...
    if header.header_crc32c != 0 {
        return Err(LybaError::checksum_mismatch(
    ...
    for section in sections {
        ...
        if section.entry.codec_id != 0 {
            return Err(LybaError::unsupported_codec(
...
fn enforce_limit_u64(value: u64, limit: usize, message: &'static str) -> Result<()> {
    if value > limit_to_u64(limit) {
        Err(LybaError::limit_exceeded(message))
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    fn limit_error(bytes: &[u8], limits: Limits) -> LybaError {
    ...
    fn reader_rejects_section_table_overlap_with_lb0006() {
        ...
        assert!(matches!(error, LybaError::OverlappingSections(_)));
    ...
    fn reader_rejects_required_unknown_section_with_lb0008() {
        ...
        assert!(matches!(error, LybaError::UnsupportedRequiredSection(_)));
    ...
    fn reader_rejects_invalid_utf8_strings_with_lb0013() {
        ...
        assert!(matches!(error, LybaError::InvalidUtf8(_)));
    ...
    fn level1_minimal_reader_round_trips_portable_documents() {
        let values = vec![
            ...
            LymaValue::String(String::from("hello")),
        ];
