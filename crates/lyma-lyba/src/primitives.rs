    ...
    fn decode_with_canonicality(
        ...
        loop {
            let byte = *input.get(*offset).ok_or_else(|| {
                at_offset(LybaError::invalid_varint("truncated unsigned varint"))
            })?;

            if len == encoded.len() {
                return Err(at_offset(LybaError::invalid_varint(
            ...
            if byte & 0x80 == 0 {
                if canonicality == VarintCanonicality::Canonical {
                    ...
                    if canonical.as_slice() != &encoded[..len] {
                        return Err(at_offset(LybaError::non_canonical_encoding(
...
pub fn read_bounded_bytes<'a>(
    ...
    if len > max_len {
        return Err(at_offset(
            LybaError::resource_limit_exceeded(format!(
    ...
    let end = (*offset).checked_add(len).ok_or_else(|| {
        at_offset(
            LybaError::offset_outside_file("byte slice length overflowed offset"),
            start,
        )
    })?;
    ...
    let bytes = input.get(*offset..end).ok_or_else(|| {
        at_offset(
            LybaError::offset_outside_file("byte slice extends beyond available input"),
            start,
        )
    })?;
...
pub fn read_bounded_str<'a>(
    ...
    str::from_utf8(bytes).map_err(|_| {
        at_offset(
            LybaError::invalid_utf8("string bytes were not valid UTF-8"),
            *offset - len,
        )
    })
}
...
pub fn read_zero_padding(input: &[u8], offset: &mut usize, len: usize) -> Result<()> {
    ...
    if padding.iter().all(|byte| *byte == 0) {
        ...
        Err(at_offset(
            LybaError::invalid_reserved_flags("padding bytes must be zero"),
            start,
        ))
    }
}
...
fn read_fixed<const N: usize>(input: &[u8], offset: &mut usize) -> Result<[u8; N]> {
    ...
    let end = (*offset).checked_add(N).ok_or_else(|| {
        at_offset(
            LybaError::offset_outside_file("fixed-width integer length overflowed offset"),
            start,
        )
    })?;
    let bytes = input.get(*offset..end).ok_or_else(|| {
        at_offset(
            LybaError::offset_outside_file("fixed-width integer extends beyond available input"),
            start,
        )
    })?;
...
fn at_offset(error: LybaError, byte_offset: usize) -> LybaError {
...
mod tests {
    ...
    fn primitives_uvar_rejects_more_than_ten_bytes() {
        ...
        assert!(matches!(error, LybaError::InvalidVarint(_)));
