...
use crate::container::{
    ContainerFooter, ContainerHeader, FOOTER_LEN, HEADER_SIZE, HeaderCrcMode, LybaFile,
};
use crate::diagnostic::{DiagnosticTable, encode_diagnostic_table};
use crate::document::Document;
use crate::document::encode_document_table;
use crate::error::{LybaError, Result};
...
use lyma_syntax::{LymaKey, LymaValue};
...
/// Writer entry point for encoding LYBA documents.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct Writer {
...
impl Writer {
    ...
    pub fn write(&self, file: &LybaFile) -> Result<Vec<u8>> {
        ...
        for document in &mut documents {
            if let Some(root_value) = &document.root_value {
                let position = original_value_roots
                    ...
                    .ok_or_else(|| {
                        LybaError::InvalidValueReference(crate::error::ErrorContext::new(
            ...
            (!extension_table.is_empty()).then_some(extension_table.clone())
        {
            let string_table = string_table.as_ref().ok_or_else(|| {
                LybaError::MalformedExtensionPayload(crate::error::ErrorContext::new(
        ...
        if !value_roots.is_empty() || self.options.mode.force_values() {
            ...
            let value_count = UVar::decode(&payload, &mut payload_offset)
                .map_err(|_| {
                    LybaError::invalid_section_table("writer produced invalid VALS payload")
                })?
                .0;
            sections.push((
                SectionEntry {
        ...
        if let Some(tag_table) = (!tag_table.is_empty()).then_some(tag_table.clone()) {
            let string_table = string_table.as_ref().ok_or_else(|| {
                LybaError::InvalidSectionTable(crate::error::ErrorContext::new(
        ...
        let entry_bytes =
            u64::try_from(encode_section_table_len(sections.len())?).map_err(|_| {
                LybaError::invalid_section_table("section table length overflowed u64")
            })?;
        let table_offset = u64::from(HEADER_SIZE);
        let mut payload_offset = table_offset
            .checked_add(entry_bytes)
            .ok_or_else(|| LybaError::invalid_section_table("section table end overflowed u64"))?;
        ...
        for (entry, payload) in &mut sections {
            ...
            payload_offset = payload_offset
                ...
                .ok_or_else(|| {
                    LybaError::invalid_section_table("section payload range overflowed u64")
                })?;
            *payload = padded;
        }
        ...
        let capacity = usize::from(HEADER_SIZE)
            ...
            .ok_or_else(|| LybaError::invalid_section_table("output length overflowed usize"))?;
        let header = header.encode(self.options.header_crc_mode)?;
...
pub(crate) fn write_level1_minimal_value_image(values: &[LymaValue]) -> Result<Vec<u8>> {
    let mut file = LybaFile::new().with_string_table(collect_level1_minimal_strings(values));
    for value in values {
        file = file
...
fn collect_level1_minimal_strings(values: &[LymaValue]) -> crate::string_table::StringTable {
...
fn runtime_metadata_for_mode(file: &LybaFile, mode: WriterMode) -> Option<Metadata> {
...
fn default_metadata_for_mode(mode: WriterMode) -> Metadata {
    let mut metadata = Metadata::new()
        ...
        .with_entry("lyma_version", Value::String(String::from("0.1")))
        .with_entry("lyba_version", Value::String(String::from("0.1")));
...
fn collect_value_strings(value: &LymaValue, interner: &mut StringInterner) {
    match value {
...
mod tests {
    ...
    use crate::value::Value;
    use lyma_syntax::{LymaKey, LymaMapping, LymaMappingEntry, LymaSequence, LymaValue};

    #[test]
    fn writer_emits_valid_minimal_header_only_file() {
        let bytes = Writer::new(WriteOptions::new())
            .write(&crate::container::LybaFile::new())
    ...
    fn writer_emits_empty_strs_section_when_requested() {
        let file = crate::container::LybaFile::new().with_string_table(StringTable::new());
    ...
    fn writer_emits_docs_and_root_document_count_for_single_document() {
        let file = crate::container::LybaFile::new()
    ...
    fn level1_minimal_writer_emits_zero_header_crc_and_only_strs_vals_docs() {
        let bytes = super::write_level1_minimal_value_image(&[LymaValue::Mapping(LymaMapping {
            entries: vec![LymaMappingEntry {
                key: LymaKey::String(String::from("name")),
                value: LymaValue::Sequence(LymaSequence {
                    items: vec![LymaValue::String(String::from("Ada"))],
                    span: None,
                }),
                span: None,
            }],
    ...
    fn runtime_data_writer_uses_existing_metadata_when_present() {
        let file = crate::container::LybaFile::new().with_metadata(
