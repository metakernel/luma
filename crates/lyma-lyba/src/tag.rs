...
pub(crate) fn decode_tag_table(
    ...
    let tag_count = usize::try_from(tag_count)
        .map_err(|_| LybaError::limit_exceeded("tag count exceeds configured maximum"))?;
...
fn encode_schema_ref(
    ...
    schema_ref
        .checked_add(1)
        .ok_or_else(|| LybaError::invalid_section_table("tag schema reference overflowed u64"))
}
    ...
    values
        ...
        .and_then(|index| {
            u64::try_from(index)
                ...
                .ok_or_else(|| {
                    LybaError::invalid_section_table("tag value reference overflowed u64")
                })
        })
}
