...
/// Convenience result alias for LYBA operations.
pub type Result<T, E = LybaError> = core::result::Result<T, E>;

/// Shared context for a structured LYBA error.
...
/// Top-level error for LYBA operations.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LybaError {
    /// LB0001: file header magic did not match the LYBA signature.
    ...
    /// Native byte value is not supported by the portable `lyma_syntax::LymaValue` boundary.
    UnsupportedByteValue(ErrorContext),
    /// Native decimal value is not supported by the portable `lyma_syntax::LymaValue` boundary.
    UnsupportedDecimalValue(ErrorContext),
    /// Runtime value is not supported by the portable `lyma_syntax::LymaValue` boundary.
...
impl fmt::Display for LybaError {
...
impl std::error::Error for LybaError {}

#[cfg(test)]
mod tests {
    use super::{ErrorContext, LybaError};
    use crate::diagnostic::{DiagnosticClass, Severity};

    #[test]
    fn code_strings_match_expected_spec_values() {
        let cases = [
            (LybaError::invalid_magic("bad magic"), "LB0001"),
            (LybaError::unsupported_version("version 9"), "LB0002"),
            (LybaError::invalid_varint("malformed varint"), "LB0012"),
            (LybaError::unsupported_codec("codec zstd"), "LB0010"),
            (
                LybaError::non_canonical_encoding("non-canonical integer"),
                "LB0017",
            ),
            (
                LybaError::limit_exceeded("document size exceeds configured maximum"),
                "LB0018",
            ),
            (
                LybaError::trusted_only_rejected("trusted-only section present"),
                "LB0019",
            ),
            (
                LybaError::invalid_reserved_flags("reserved flags set"),
                "LB0025",
            ),
            (
                LybaError::unsupported_byte_value("bytes are native-only"),
                "LB0019",
            ),
            (
                LybaError::unsupported_decimal_value("decimal values are native-only"),
                "LB0024",
            ),
            (
                LybaError::unsupported_runtime_value("runtime values are not portable"),
                "LB0019",
            ),
        ];
    ...
    fn metadata_and_classification_are_preserved_in_diagnostics() {
        let error = LybaError::invalid_varint("malformed varint").with_context(
    ...
    fn policy_errors_are_classified_as_policy() {
        let error = LybaError::trusted_only_rejected("trusted-only section present");
    ...
    fn unsupported_runtime_values_are_classified_as_policy() {
        let error = LybaError::unsupported_runtime_value("runtime values are not portable");
