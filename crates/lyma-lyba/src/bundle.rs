...
/// LYMA `@import` dependency.
pub const DEPENDENCY_KIND_IMPORT: u64 = 0;
/// LYMA `@include` dependency.
...
/// External non-LYMA resource dependency.
pub const DEPENDENCY_KIND_EXTERNAL_RESOURCE: u64 = 6;
/// Extension-defined dependency.
pub const DEPENDENCY_KIND_EXTENSION: u64 = 7;

/// Embedded LYMA text source.
pub const EMBEDDED_RESOURCE_KIND_LYMA_TEXT: u64 = 0;
/// Embedded nested LYBA container.
pub const EMBEDDED_RESOURCE_KIND_LYBA_CONTAINER: u64 = 1;
/// Embedded schema written in LYMA text.
pub const EMBEDDED_RESOURCE_KIND_SCHEMA_LYMA: u64 = 2;
...
impl EmbeddedResourceRecord {
    ...
    pub fn utf8_text<'a>(&self, blob_table: &'a BlobTable) -> Result<Option<&'a str>> {
        ...
        core::str::from_utf8(blob.as_bytes())
            .map(Some)
            .map_err(|_| LybaError::invalid_utf8("embedded resource text was not valid UTF-8"))
    }
}
...
pub(crate) fn decode_dependency_table(
    ...
    let dependency_count = usize::try_from(UVar::decode(payload, &mut offset)?.0)
        .map_err(|_| LybaError::limit_exceeded("dependency count exceeds configured maximum"))?;
...
pub(crate) fn decode_embedded_resource_table(
    ...
    let resource_count = usize::try_from(UVar::decode(payload, &mut offset)?.0).map_err(|_| {
        LybaError::limit_exceeded("embedded resource count exceeds configured maximum")
    })?;
...
fn validate_embedded_resource_record(
    ...
    match (kind, extension_kind) {
        (EMBEDDED_RESOURCE_KIND_EXTENSION, None) => Err(LybaError::InvalidSectionTable(
        ...
        (_, Some(_)) => Err(LybaError::InvalidSectionTable(
            ErrorContext::new(
    ...
    strings
        ...
        .and_then(|index| {
            u64::try_from(index)
                ...
                .ok_or_else(|| {
                    LybaError::invalid_section_table("dependency string reference overflowed u64")
                })
        })
}
...
fn encode_source_span_ref(
    ...
    source_span_ref.checked_add(1).ok_or_else(|| {
        LybaError::invalid_section_table("dependency source span reference overflowed u64")
    })
}
...
fn encode_blob_ref(
    ...
    blob_ref.0.checked_add(1).ok_or_else(|| {
        LybaError::invalid_section_table("dependency blob reference overflowed u64")
    })
}
    ...
    values
        ...
        .and_then(|index| {
            u64::try_from(index)
                ...
                .and_then(|index| index.checked_add(1))
                .ok_or_else(|| {
                    LybaError::invalid_section_table("dependency value reference overflowed u64")
                })
        })
}
