...
fn source_spans_for(parsed: &lyma_parser::Parsed) -> SourceSpanTable {
...
fn astn_round_trips_parsed_documents_and_preserves_preorder() {
    ...
    let mut syntax_nodes = SyntaxNodeTable::from_lyma_file(&parsed.file, Some(&source_spans));
...
fn astn_invalid_node_refs_use_lb0015() {
    let file = LybaFile::new().with_syntax_node_table(
