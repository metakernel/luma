...
use lyma_lyba::LybaFile;
use lyma_lyba::container::{
    ContainerFooter, ContainerHeader, FOOTER_LEN, HEADER_SIZE, HeaderCrcMode, SECTION_ENTRY_SIZE,
};
use lyma_lyba::document::Document;
use lyma_lyba::error::LybaError;
use lyma_lyba::read::{ReadOptions, Reader};
use lyma_lyba::section::{CHECKSUM_CRC32C, CHECKSUM_NONE, SectionEntry};
use lyma_lyba::value::Value;
use lyma_lyba::write::{WriteOptions, Writer};

fn sample_file() -> LybaFile {
    LybaFile::new()
...
fn checksum_mismatch_returns_lb0011() {
    ...
    assert!(matches!(error, LybaError::ChecksumMismatch(_)));
