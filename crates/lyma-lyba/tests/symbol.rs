...
use lyma_lyba::value::{MapEntry, TaggedValue, Value};
use lyma_lyba::{
    Document, LybaError, LybaFile, ReadOptions, Reader, WriteOptions, Writer, WriterMode,
};
    ...
    let file = LybaFile::new()
        .with_string_table(
            StringTable::new()
                ...
                .with_string(StringRecord::new("lyma")),
        )
        .with_symbol_table(
            SymbolTable::new()
...
fn runtime_data_writer_emits_deterministic_symbol_ordering() {
    let file = LybaFile::new().with_document(Document::new().with_root_value(Value::Map(vec![
        MapEntry {
