...
use lyma_lyba::{
    BlobId, BlobRecord, BlobTable, Limits, LybaError, LybaFile, ReadOptions, Reader,
...
fn source_digest_blob_ref_round_trips() {
    ...
    let file = LybaFile::new()
        ...
        .with_source_file_table(
            SourceFileTable::new().with_record(
                SourceFileRecord::new()
                    .with_uri(Some(String::from("urn:lyma:source:1")))
...
fn private_source_policy_uses_lb0019() {
    let file = LybaFile::new().with_source_file_table(
        SourceFileTable::new().with_record(
            SourceFileRecord::new()
...
fn writer_rejects_reserved_source_flags_with_lb0025() {
    let file = LybaFile::new().with_source_file_table(SourceFileTable::new().with_record(
