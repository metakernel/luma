...
use lyma_syntax::{FileId, SourcePosition, Span};
...
fn source_spans_round_trip_and_convert_when_data_is_available() {
    ...
    assert_eq!(span.to_lyma_syntax_span(), Some(Span::new(FileId(0), 0, 5)));
    assert_eq!(
        span.start_position_to_lyma_syntax(),
        Some(SourcePosition { line: 1, column: 1 })
    );
    assert_eq!(
        span.end_position_to_lyma_syntax(),
        Some(SourcePosition { line: 1, column: 6 })
    );

    let rebuilt = SourceSpanRecord::from_lyma_syntax(
