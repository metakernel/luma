...
use lyma_syntax::{
    Diagnostic, DiagnosticCode, FileId, LymaNode, NumberNode, Span, StringNode, StringStyle,
};
...
pub(crate) fn parse_inline_scalar(
    ...
    if text.is_empty() || matches!(text, "null" | "nil") {
        return LymaNode::Null {
            span: Span::new(file_id, start, start + text.len()),
        };
    }
    if text == "true" {
        return LymaNode::Boolean {
            value: true,
            span: Span::new(file_id, start, start + text.len()),
        };
    }
    ...
    if let Some((value, style)) = parse_quoted_string(text, diagnostics, file_id, start) {
        return LymaNode::String(StringNode {
            value,
    ...
    if text.starts_with('=') {
        return LymaNode::LuaExpression(inline_expression(text, start, file_id));
    }
    if text.starts_with('{') && text.ends_with('}') {
        return LymaNode::LuaTableConstructor(inline_table(text, start, file_id));
    }
    ...
    if is_lua_number(text) {
        return LymaNode::Number(NumberNode {
            lexeme: text.to_owned(),
            span: Span::new(file_id, start, start + text.len()),
        });
    }

    LymaNode::String(StringNode {
        value: text.to_owned(),
        source: text.to_owned(),
        style: StringStyle::Plain,
        block_kind: None,
...
#[cfg(test)]
mod tests {
    use lyma_syntax::{DiagnosticCode, FileId, LymaNode};
