//! Structural lexer for normalized Lyma source.

use lyma_syntax::{BlockKind, Diagnostic, DiagnosticCode, FileId, Span, Token, TokenKind};
...
#[cfg(test)]
mod tests {
    use lyma_syntax::{BlockKind, DiagnosticCode, FileId, TokenKind};

    use super::lex_str;

    #[test]
    fn normalizes_spec_section_5_encoding_rules() {
        let lexed = lex_str(FileId(1), "spec.lyma", "\u{feff}---\r\n...");
    ...
    fn treats_hash_as_plain_text() {
        let lexed = lex_str(FileId(1), "spec.lyma", "label: Section #1\n");
        assert!(lexed.diagnostics.is_empty());
    ...
    fn emits_e0003_for_tabs_used_as_indentation() {
        let lexed = lex_str(FileId(1), "spec.lyma", "root:\n\tchild: value\n");
        assert!(
            lexed
                .diagnostics
                .iter()
    ...
    fn emits_e0004_for_unterminated_strings() {
        let lexed = lex_str(FileId(1), "spec.lyma", "message: \"oops\n");
        assert!(
            lexed
                .diagnostics
                .iter()
    ...
    fn emits_e0005_for_unterminated_block_comments() {
        let lexed = lex_str(FileId(1), "spec.lyma", "--[[\ncomment\n");
        assert!(
            lexed
                .diagnostics
                .iter()
    ...
    fn emits_e0026_for_missing_block_content() {
        let lexed = lex_str(FileId(1), "spec.lyma", "script: |expr\nnext: value\n");
        assert!(
            lexed
                .diagnostics
                .iter()
    ...
    fn records_whitespace_comment_and_newline_trivia_spans() {
        ...
        let lexed = lex_str(FileId(1), "spec.lyma", source);
