//! Canonical formatter for parsed Lyma syntax trees.

use lyma_syntax::{
    BlockChomping, BlockKind, Comment, CommentKind, ConditionalBlock, Directive, Document,
    DocumentItem, ElseBranch, LetBinding, LoopBindings, LoopBlock, LymaFile, LymaNode,
...
/// Formats parsed input into canonical Lyma text.
#[must_use]
...
/// Parses and formats source text into canonical Lyma text.
#[must_use]
...
/// Parses and formats a source range into canonical Lyma edits.
///
...
#[must_use]
pub fn format_file(file: &LymaFile, options: FormatOptions) -> String {
...
fn render_file(file: &LymaFile, options: FormatOptions) -> String {
...
fn render_node(node: &LymaNode, depth: usize, out: &mut String, options: FormatOptions) {
    match node {
        LymaNode::Null { .. } => push_line(depth, options, "null", out),
        LymaNode::Boolean { value, .. } => {
            push_line(depth, options, if *value { "true" } else { "false" }, out);
        }
        LymaNode::Number(number) => push_line(depth, options, &number.lexeme, out),
        LymaNode::String(string) => render_string_node(string, depth, out, options),
        LymaNode::Sequence(sequence) => render_sequence_block(sequence, depth, out, options),
        LymaNode::Mapping(mapping) => render_mapping_block(mapping, depth, out, options),
...
fn render_directive(directive: &Directive, depth: usize, out: &mut String, options: FormatOptions) {
    match directive {
        Directive::Version(version) => {
            push_line(depth, options, &format!("@lyma {}", version.version), out);
        }
...
fn inline_node(node: &LymaNode) -> Option<String> {
    match node {
        LymaNode::Null { .. } => Some(String::from("null")),
        LymaNode::Boolean { value, .. } => Some(if *value {
            String::from("true")
        } else {
            String::from("false")
        }),
        LymaNode::Number(number) => Some(number.lexeme.clone()),
        LymaNode::String(string)
            if string.style != StringStyle::Block && !string.value.contains('\n') =>
        {
            Some(format_string_scalar(&string.value))
        }
        LymaNode::Tagged(tagged) => tagged
            .value
            .as_deref()
            .and_then(inline_node)
            .map(|value| format!("!{} {value}", tagged.tag.name.value))
            .or_else(|| Some(format!("!{}", tagged.tag.name.value))),
        LymaNode::LuaExpression(expression) => Some(format!("={}", expression.source.trim())),
        LymaNode::LuaTableConstructor(expression) => Some(expression.source.trim().to_owned()),
        _ => None,
    }
}
...
fn profile_name(profile: &lyma_syntax::LymaProfile) -> &str {
    match profile {
        lyma_syntax::LymaProfile::Data => "data",
        lyma_syntax::LymaProfile::Safe => "safe",
        lyma_syntax::LymaProfile::Trusted => "trusted",
        lyma_syntax::LymaProfile::Custom(value) => value,
    }
}
...
mod tests {
    ...
    fn unchanged_formatting_returns_no_text_edits() {
        let source = "root:\n  value: hello\n";
        let formatted = format_str(FileId(1), "stable.lyma", source);
    ...
    fn one_line_changes_produce_small_replacements() {
        let source = "root:\n  value: 'hello'\n";
        let formatted = format_str(FileId(1), "single-line.lyma", source);
    ...
    fn multi_line_changes_expand_to_line_boundaries() {
        let source = "root:\n    alpha: 1\r\n    beta: 'two'\n";
        let formatted = format_str(FileId(1), "multi-line.lyma", source);
        let edits = formatted.text_edits_for_source(source);
    ...
    fn range_formatting_expands_nested_block_ranges() {
        ...
        let parsed = parse_str(FileId(1), "range-block.lyma", source);
        let edits =
