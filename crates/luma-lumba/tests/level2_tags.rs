...
use luma_lumba::{
    Document, LumbaError, LumbaFile, ReadOptions, Reader, TAG_FLAG_KNOWN_TO_PRODUCER,
...
fn tag_registry_round_trips_flags_metadata_and_tagged_values_without_resolution() {
    ...
    assert_eq!(declaration.uri, "urn:luma:example:duration");
...
fn trusted_only_tag_requires_trusted_reader_policy() {
    let file = LumbaFile::new().with_tag_table(TagTable::new().with_declaration(
        TagDeclaration::new("Secret", "urn:luma:example:secret").with_flags(TAG_FLAG_TRUSTED_ONLY),
    ));
...
fn reader_rejects_non_tag_symbol_reference_with_lb0014() {
    let strs = strs_payload(&[b"Duration", b"urn:luma:example:duration"]);
