...
use luma_lumba::symbol::SymbolRecord;
use luma_lumba::{
    ...
    DEPENDENCY_KIND_SOURCE, DependencyRecord, DependencyTable, Limits, LumbaError, LumbaFile,
        ...
        DependencyTable::new()
            .with_record(
                DependencyRecord::new(DEPENDENCY_KIND_IMPORT)
                    .with_uri(Some(String::from("https://example.invalid/import.luma")))
            ...
            .with_record(
                DependencyRecord::new(DEPENDENCY_KIND_INCLUDE)
                    .with_uri(Some(String::from("file:///tmp/include.luma")))
            ...
            .with_record(
                DependencyRecord::new(DEPENDENCY_KIND_SCHEMA)
                    .with_uri(Some(String::from("urn:luma:schema:service"))),
            )
            .with_record(
                DependencyRecord::new(DEPENDENCY_KIND_SOURCE)
                    .with_uri(Some(String::from("file:///workspace/main.luma"))),
            )
            .with_record(
...
fn dependency_uris_are_inert_and_do_not_trigger_resolution() {
    let file = fixture_file().with_dependency_table(
        DependencyTable::new()
            .with_record(
                DependencyRecord::new(DEPENDENCY_KIND_IMPORT)
                    .with_uri(Some(String::from("file:///definitely/not/read.luma"))),
            )
            .with_record(
                DependencyRecord::new(DEPENDENCY_KIND_EXTERNAL_RESOURCE)
    ...
    assert_eq!(
        ...
        Some("file:///definitely/not/read.luma")
    );
    assert_eq!(
...
fn reader_rejects_invalid_dependency_uri_and_alias_refs_with_lb0014() {
    ...
    assert!(matches!(uri_error, LumbaError::InvalidValueReference(_)));
    ...
    assert!(matches!(alias_error, LumbaError::InvalidValueReference(_)));
...
fn patch_deps_payload(bytes: &mut [u8], payload: &[u8]) {
    ...
    for index in 0..header.section_count as usize {
        ...
        if entry.section_id == luma_lumba::section::SectionId::DEPS {
