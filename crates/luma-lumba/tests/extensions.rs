...
use luma_lumba::{
    ...
    LumbaError, LumbaFile, ReadOptions, Reader, Value, WriteOptions, Writer,
};

#[test]
fn unsupported_required_extension_fails_with_lb0009() {
    let file = LumbaFile::new().with_extension_table(
        ExtensionTable::new().with_declaration(
            ExtensionDeclaration::new("org.luma.lua.bytecode.lua54", "1.0")
    ...
    assert!(matches!(error, LumbaError::UnsupportedRequiredExtension(_)));
...
fn trusted_only_extension_fails_under_public_policy() {
    let file = LumbaFile::new().with_extension_table(ExtensionTable::new().with_declaration(
