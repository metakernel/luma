...
use luma_lumba::symbol::{SYMBOL_FLAG_TAG, SymbolRecord, SymbolTable};
use luma_lumba::{
    BlobId, BlobRecord, BlobTable, Document, LumbaError, LumbaFile, ReadOptions, Reader,
...
fn schema_uri_only_round_trips_as_inert_data() {
    let file = LumbaFile::new().with_schema_table(
    ...
    assert_eq!(schema.uri.as_deref(), Some("urn:luma:schema:service"));
    ...
    let file = LumbaFile::new()
        .with_string_table(
            StringTable::new()
                ...
                .with_string(StringRecord::new("urn:luma:tag:duration"))
                .with_string(StringRecord::new("urn:luma:schema:duration")),
        )
        .with_symbol_table(
            SymbolTable::new().with_symbol(SymbolRecord::new(0).with_flags(SYMBOL_FLAG_TAG)),
        )
        .with_schema_table(SchemaTable::new().with_record(
            SchemaRecord::new().with_uri(Some(String::from("urn:luma:schema:duration"))),
        ))
        .with_tag_table(TagTable::new().with_declaration(
            TagDeclaration::new("Duration", "urn:luma:tag:duration").with_schema_ref(Some(0)),
        ));
...
fn trusted_validator_schema_is_rejected_under_public_policy() {
    ...
    assert!(
        Reader::new(ReadOptions::new().with_limits(luma_lumba::Limits::trusted()))
            .read(&bytes)
            .is_ok()
    );
}
