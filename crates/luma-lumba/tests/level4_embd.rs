...
use luma_lumba::container::{ContainerHeader, HeaderCrcMode};
...
use luma_lumba::{
    ...
    EMBEDDED_RESOURCE_KIND_SCHEMA_LUMA, EmbeddedResourceRecord, EmbeddedResourceTable, Limits,
    LumbaError, LumbaFile, ReadOptions, Reader, WriteOptions, Writer,
};
...
fn embedded_resources_round_trip_text_binary_lazy_access_and_inert_lua() {
    ...
    assert_eq!(resources[0].kind, EMBEDDED_RESOURCE_KIND_LUMA_TEXT);
    assert_eq!(resources[1].kind, EMBEDDED_RESOURCE_KIND_LUMBA_CONTAINER);
    assert_eq!(resources[2].kind, EMBEDDED_RESOURCE_KIND_SCHEMA_LUMA);
...
fn writer_rejects_invalid_embedded_dependency_and_blob_refs_with_lb0014() {
    ...
    assert!(matches!(blob_error, LumbaError::InvalidValueReference(_)));
    ...
    LumbaFile::new()
            ...
            DependencyTable::new()
                .with_record(
                    DependencyRecord::new(DEPENDENCY_KIND_IMPORT)
                        .with_uri(Some(String::from("mem://text.luma")))
                        .with_flags(DEPENDENCY_FLAG_EMBEDDED),
                )
                .with_record(
                    DependencyRecord::new(DEPENDENCY_KIND_GENERATED)
                        .with_uri(Some(String::from("mem://nested.lumba")))
                        .with_flags(DEPENDENCY_FLAG_EMBEDDED),
                )
                .with_record(
                    DependencyRecord::new(DEPENDENCY_KIND_IMPORT)
                        .with_uri(Some(String::from("mem://schema.luma")))
                        .with_flags(DEPENDENCY_FLAG_EMBEDDED),
                )
                .with_record(
        ...
        .with_embedded_resource_table(
            EmbeddedResourceTable::new()
                ...
                .with_record(EmbeddedResourceRecord::new(
                    1,
                    EMBEDDED_RESOURCE_KIND_LUMBA_CONTAINER,
                    BlobId(1),
                ))
                .with_record(EmbeddedResourceRecord::new(
                    2,
...
fn patch_embd_payload(bytes: &mut [u8], payload: &[u8]) {
    ...
    for index in 0..header.section_count as usize {
        ...
        if entry.section_id == luma_lumba::section::SectionId::EMBD {
