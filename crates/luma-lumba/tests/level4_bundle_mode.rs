...
use luma_lumba::extension::{ExtensionDeclaration, ExtensionTable};
use luma_lumba::primitives::Identifier;
use luma_lumba::schema::{SCHEMA_FLAG_REQUIRED_BY_DOCUMENT, SchemaRecord, SchemaTable};
use luma_lumba::section::SectionEntry;
use luma_lumba::signature::SignatureVerifier;
...
use luma_lumba::{
    ...
    Document, EMBEDDED_RESOURCE_KIND_LUMA_TEXT, EmbeddedResourceRecord, EmbeddedResourceTable,
    LumbaFile, ReadOptions, Reader, SIGNATURE_ALGORITHM_SHA256, SIGNATURE_RECORD_KIND_DIGEST,
...
fn build_bundle_mode_emits_bundle_sections_and_keeps_dependencies_inert() {
    ...
    let extension = &decoded
        ...
        .declarations[0];
    assert_eq!(extension.name, "org.luma.bundle");
    assert_eq!(
    ...
    let schema = &decoded
        ...
        .records[0];
    assert_eq!(schema.uri.as_deref(), Some("urn:luma:schema:bundle"));
    ...
    assert_eq!(resource.kind, EMBEDDED_RESOURCE_KIND_LUMA_TEXT);
    assert_eq!(
        resource
    ...
    LumbaFile::new()
        ...
        .with_extension_table(
            ExtensionTable::new().with_declaration(
                ExtensionDeclaration::new("org.luma.bundle", "1.0.0")
        ...
        .with_schema_table(
            SchemaTable::new().with_record(
                SchemaRecord::new()
                    ...
                    .with_uri(Some(String::from("urn:luma:schema:bundle")))
        ...
        .with_embedded_resource_table(EmbeddedResourceTable::new().with_record(
            EmbeddedResourceRecord::new(0, EMBEDDED_RESOURCE_KIND_LUMA_TEXT, BlobId(0)),
        ))
