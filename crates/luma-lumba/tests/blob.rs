...
use luma_lumba::{
    ...
    BLOB_FLAG_UTF8_TEXT, BlobId, BlobRecord, BlobTable, CanonicalMode, LumbaError, LumbaFile,
    ReadOptions, Reader, Value, WriteOptions, Writer, WriterMode,
};

fn value_section(values: Vec<Value>) -> luma_lumba::section::Section {
    let mut section = luma_lumba::section::Section::new("VALS");
    for value in values {
...
fn write_then_read(file: &LumbaFile, mode: WriterMode) -> luma_lumba::Result<LumbaFile> {
...
fn canonical_writer_keeps_small_bytes_inline() {
    let file = LumbaFile::new().with_section(value_section(vec![Value::BytesInline(vec![7; 64])]));
...
fn canonical_writer_promotes_large_inline_bytes_into_blob_section() {
    let file = LumbaFile::new().with_section(value_section(vec![Value::BytesInline(vec![9; 65])]));
...
fn writer_rejects_invalid_blob_refs_with_lb0014() {
    let file = LumbaFile::new().with_section(value_section(vec![Value::BytesBlob(BlobId(0))]));
