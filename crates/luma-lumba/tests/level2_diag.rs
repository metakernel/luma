...
use luma_lumba::symbol::{SymbolRecord, SymbolTable, encode_symbol_table};
use luma_lumba::{
    DiagnosticLoadPolicy, DiagnosticRecord, DiagnosticTable, LumbaError, LumbaFile, ReadOptions,
...
use luma_syntax::{RelatedDiagnosticSpan, Span};
...
fn parse_warning_fixture_round_trips() {
    let mut parsed = parse_str(FileId(7), "warning.luma", "\tname: value\n");
    assert!(!parsed.diagnostics.is_empty());
    parsed.diagnostics[0].severity = Severity::Warning;

    let record = DiagnosticRecord::from_luma_syntax(&parsed.diagnostics[0]);
    let file = LumbaFile::new().with_diagnostic_table(DiagnosticTable::new().with_record(record));
...
fn error_bearing_load_is_accepted_or_rejected_by_policy() {
    let file = LumbaFile::new().with_diagnostic_table(DiagnosticTable::new().with_record(
    ...
    let warning_file = LumbaFile::new().with_diagnostic_table(DiagnosticTable::new().with_record(
        DiagnosticRecord::new(
    ...
    assert!(matches!(warning_error, LumbaError::TrustedOnlyRejected(_)));
}
...
fn conversion_helpers_preserve_related_spans_when_mapping_is_available() {
    ...
    let stored = DiagnosticRecord::from_luma_syntax_with_span_encoder(&diagnostic, |span| {
        if span == primary {
            Some(0)
    ...
    let round_tripped = stored
        .to_luma_syntax_with_span_resolver(|span_ref| match span_ref {
            0 => Some(primary),
            1 => Some(related),
            _ => None,
        })
...
fn diag_reserved_flags_are_rejected_with_lb0025_on_write() {
    let file = LumbaFile::new().with_diagnostic_table(
