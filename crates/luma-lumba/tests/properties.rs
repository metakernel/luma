...
use luma_lumba::primitives::{Identifier, SVar, UVar};
use luma_lumba::section::{CHECKSUM_NONE, CODEC_NONE, Section, SectionEntry, SectionId};
use luma_lumba::{
...
fn arb_luma_number() -> impl Strategy<Value = LumaNumber> {
    prop_oneof![
        any::<i64>().prop_map(LumaNumber::Integer),
        any::<u64>()
            .prop_map(f64::from_bits)
            .prop_filter("finite float", |value| value.is_finite())
            .prop_map(LumaNumber::Float),
    ]
}

fn arb_luma_value() -> BoxedStrategy<LumaValue> {
    let leaf = prop_oneof![
        Just(LumaValue::Null(LumaNull)),
        any::<bool>().prop_map(LumaValue::Boolean),
        arb_luma_number().prop_map(LumaValue::Number),
        arb_string(16).prop_map(LumaValue::String),
    ];

    leaf.prop_recursive(MAX_DEPTH, 64, 8, |inner| {
        prop_oneof![
            prop::collection::vec(inner.clone(), 0..=4)
                .prop_map(|items| { LumaValue::Sequence(LumaSequence { items, span: None }) }),
            prop::collection::btree_map(arb_string(8), inner.clone(), 0..=4).prop_map(|entries| {
                LumaValue::Mapping(LumaMapping {
                    entries: entries
                        .into_iter()
                        .map(|(key, value)| LumaMappingEntry {
...
fn arb_native_value() -> BoxedStrategy<Value> {
    let leaf = prop_oneof![
        ...
        (0_u64..=1).prop_map(|id| Value::BytesBlob(luma_lumba::BlobId(id))),
    ];
...
proptest! {
    ...
    #[test]
    fn portable_value_images_round_trip(values in prop::collection::vec(arb_luma_value(), 0..=6)) {
        let encoded = luma_lumba::try_to_lumba_value_image(&values)?;
        let decoded = luma_lumba::try_from_lumba_value_image(&encoded)?;
        let reencoded = luma_lumba::try_to_lumba_value_image(&decoded)?;
    ...
    fn native_value_containers_round_trip(values in prop::collection::vec(arb_native_value(), 0..=4)) {
        ...
        let mut file = luma_lumba::LumbaFile::new().with_blob_table(blob_table);
        for value in &values {
