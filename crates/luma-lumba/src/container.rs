...
impl ContainerFooter {
    ...
    pub fn decode(input: &[u8]) -> Result<Self> {
        ...
        if major_version != HEADER_MAJOR_VERSION || minor_version != HEADER_MINOR_VERSION {
            return Err(LumbaError::UnsupportedVersion(
                ErrorContext::new(format!(
        ...
        if footer_crc32c != crc32c_footer(input) {
            return Err(LumbaError::ChecksumMismatch(
                ErrorContext::new(format!(
...
impl ContainerHeader {
    ...
    pub fn decode_with_reserved_flag_policy(
        ...
        if input.len() < usize::from(HEADER_SIZE) {
            return Err(LumbaError::OffsetOutsideFile(
        ...
        if input[..HEADER_MAGIC.len()] != HEADER_MAGIC {
            return Err(LumbaError::InvalidMagic(
                ErrorContext::new("container header magic did not match LUMBA signature")
                    .with_byte_offset(0),
            ));
        }
        ...
        if header_size != HEADER_SIZE {
            return Err(LumbaError::InvalidHeaderSize(
                ErrorContext::new(format!(
        ...
        if endian_marker != HEADER_ENDIAN_MARKER {
            return Err(LumbaError::InvalidEndianMarker(
                ErrorContext::new(format!(
...
pub fn validate_section_table_with_reserved_flag_policy<'a>(
        ...
        .enumerate()
    {
        ...
        match supported_section_semantics(entry.section_id) {
            Some(semantics) => {
                if entry.section_version != semantics.version {
                    return Err(LumbaError::UnsupportedRequiredSection(
                        ErrorContext::new(format!(
        ...
        if !is_supported_codec(entry.codec_id) {
            if entry.is_required() {
                return Err(LumbaError::UnsupportedCodec(
                    ErrorContext::new(format!(
    ...
    for pair in spans.windows(2) {
        ...
        if current.end > next.start {
            return Err(LumbaError::OverlappingSections(
...
/// In-memory representation of a LUMBA file.
#[derive(Debug, Clone, PartialEq, Default)]
pub struct LumbaFile {
...
mod tests {
    ...
    fn rejects_invalid_magic_with_lb0001() {
        ...
        assert!(matches!(error, LumbaError::InvalidMagic(_)));
    ...
    fn rejects_unsupported_version_with_lb0002() {
        ...
        assert!(matches!(error, LumbaError::UnsupportedVersion(_)));
    ...
    fn rejects_invalid_endian_marker_with_lb0003() {
        ...
        assert!(matches!(error, LumbaError::InvalidEndianMarker(_)));
    ...
    fn rejects_invalid_header_size_with_lb0004() {
        ...
        assert!(matches!(error, LumbaError::InvalidHeaderSize(_)));
