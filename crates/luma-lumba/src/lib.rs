//! Opt-in LUMBA support crate for the Luma workspace.
//!
...
//! ```
//!
//! `luma-lumba` is a binary container library, not an execution engine.
//! Reading, inspecting, verifying, and writing LUMBA never executes Lua,
...
//!
//! ```
//! use luma_lumba::{CanonicalMode, Limits, LumbaFile, ReadOptions, Reader, WriteOptions, Writer, WriterMode};
//!
...
//!
//! ```
//! use luma_lumba::{Document, Limits, LumbaFile, ReadOptions, Reader, Value, WriteOptions, Writer};
//!
//! let file = LumbaFile::new()
...
//! # Ok::<(), luma_lumba::LumbaError>(())
//! ```
//!
//! Convert portable `luma_syntax::LumaValue` values with the helper API:
//!
//! ```
//! use luma_lumba::{try_from_lumba_value_image, try_to_lumba_value_image};
//! use luma_syntax::{LumaNull, LumaValue};
//!
//! let bytes = try_to_lumba_value_image(&[LumaValue::Null(LumaNull)])?;
//! let values = try_from_lumba_value_image(&bytes)?;
//!
...
/// Backward-compatible diagnostics module path for structured LUMBA spec codes.
pub mod diagnostics {
...
#[must_use]
pub fn to_lumba_value_image(values: &[LumaValue]) -> Vec<u8> {
    try_to_lumba_value_image(values).expect("to_lumba_value_image requires portable Level 1 values")
}

/// Decodes a canonical Level 1 minimal value image into portable values.
pub fn from_lumba_value_image(bytes: &[u8]) -> Result<Vec<LumaValue>> {
    try_from_lumba_value_image(bytes)
}

/// Fallible form of [`to_lumba_value_image`].
pub fn try_to_lumba_value_image(values: &[LumaValue]) -> Result<Vec<u8>> {
    write::write_level1_minimal_value_image(values)
}

/// Fallible form of [`from_lumba_value_image`].
pub fn try_from_lumba_value_image(bytes: &[u8]) -> Result<Vec<LumaValue>> {
...
pub use container::{DocumentImage, LumbaFile};
pub use diagnostic::{
    Diagnostic, DiagnosticClass, DiagnosticCode, DiagnosticLoadPolicy, DiagnosticRecord,
    DiagnosticTable, RelatedDiagnosticRecord, Severity, StoredDiagnosticSeverity,
};
...
pub use error::{ErrorContext, LumbaError, Result};
pub use extension::{
...
#[cfg(test)]
mod tests {
    use super::{from_lumba_value_image, to_lumba_value_image};
    use luma_syntax::{
        LumaKey, LumaMapping, LumaMappingEntry, LumaNull, LumaSequence, LumaTag, LumaTagName,
        LumaTaggedValue, LumaValue, source::Span,
    };

    fn span() -> Span {
        Span::new(luma_syntax::FileId(0), 0, 0)
    }
    ...
    fn level1_minimal_encode_is_byte_identical_for_same_values() {
                ...
                value: Box::new(LumaValue::Mapping(LumaMapping {
                    entries: vec![LumaMappingEntry {
                        ...
                        value: LumaValue::Sequence(LumaSequence {
                            items: vec![LumaValue::String(String::from("x"))],
                            span: None,
                        }),
                        span: None,
                    }],
        ...
        let left = to_lumba_value_image(&values);
        let right = to_lumba_value_image(&values);
    ...
    fn level1_minimal_decode_returns_original_portable_documents() {
        ...
        let decoded = from_lumba_value_image(&to_lumba_value_image(&values))
...
pub use bundle::{
    ...
    EMBEDDED_RESOURCE_KIND_LUMA_TEXT, EMBEDDED_RESOURCE_KIND_LUMBA_CONTAINER,
    EMBEDDED_RESOURCE_KIND_SCHEMA_LUMA, EMBEDDED_RESOURCE_SECTION_NAME, EmbeddedResourceRecord,
