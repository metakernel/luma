...
pub(crate) fn decode_syntax_node_table(
    ...
    let node_count = usize::try_from(UVar::decode(payload, &mut offset)?.0)
        .map_err(|_| LumbaError::limit_exceeded("syntax node count exceeds configured maximum"))?;
    ...
    let records_offset = offset
        ...
        .ok_or_else(|| {
            LumbaError::InvalidSectionTable(ErrorContext::new("ASTN offset table end overflowed"))
        })?;
    ...
    for (record_index, start) in node_offsets.iter().copied().enumerate() {
        if start < previous {
            return Err(LumbaError::NonCanonicalEncoding(
...
fn validate_node_ref(value: u64, count: usize, record_index: usize, field: &str) -> Result<()> {
    let index = usize::try_from(value).map_err(|_| {
        LumbaError::InvalidSyntaxNodeReference(
            ErrorContext::new(format!("ASTN {field} reference exceeded platform limits"))
                .with_record_index(record_index),
        )
    })?;
...
impl<'a> SpanLookup<'a> {
    ...
    fn span_ref(&self, span: luma_syntax::Span) -> Option<u64> {
...
impl<'a> SyntaxTableBuilder<'a> {
    ...
    fn push_file(&mut self, file: &luma_syntax::LumaFile, parent: Option<usize>) -> usize {
    ...
    fn push_document(&mut self, document: &luma_syntax::Document, parent: Option<usize>) -> usize {
    ...
    fn push_document_item(&mut self, item: &luma_syntax::DocumentItem, parent: Option<usize>) {
        match item {
            luma_syntax::DocumentItem::Directive(directive) => {
                self.push_directive(directive, parent);
            }
            luma_syntax::DocumentItem::Let(binding) => {
                self.push_let_binding(binding, parent);
            }
            luma_syntax::DocumentItem::Root(node) => {
                self.push_luma_node(node, parent);
            }
            luma_syntax::DocumentItem::Comment(comment) => {
                self.push_comment(comment, parent);
            }
        }
    }

    fn push_luma_node(&mut self, node: &luma_syntax::LumaNode, parent: Option<usize>) -> usize {
        match node {
            luma_syntax::LumaNode::Null { span } => {
                self.push_node("null", Some(*span), parent, Vec::new())
            }
            luma_syntax::LumaNode::Boolean { value, span } => self.push_node(
                "boolean",
                Some(*span),
                parent,
                vec![SyntaxField::new("value", SyntaxFieldValue::Bool(*value))],
            ),
            luma_syntax::LumaNode::Number(number) => self.push_node(
                "number",
                Some(number.span),
                parent,
                vec![SyntaxField::new(
            ...
            luma_syntax::LumaNode::String(string) => {
                let kind = match (string.style, string.block_kind) {
                    (luma_syntax::StringStyle::Plain, None) => "plain_scalar",
            ...
            luma_syntax::LumaNode::Sequence(sequence) => self.push_sequence(sequence, parent),
            luma_syntax::LumaNode::Mapping(mapping) => self.push_mapping(mapping, parent),
            luma_syntax::LumaNode::Tagged(tagged) => self.push_tagged(tagged, parent),
            luma_syntax::LumaNode::LuaExpression(expression) => {
                self.push_lua_expression("lua_expression", expression, parent)
            }
            luma_syntax::LumaNode::LuaExpressionBlock(expression) => {
                self.push_lua_expression("lua_expression_block", expression, parent)
            }
            luma_syntax::LumaNode::LuaChunk(expression) => {
                self.push_lua_expression("lua_chunk", expression, parent)
            }
            luma_syntax::LumaNode::LuaTableConstructor(expression) => {
    ...
    fn push_mapping(
        ...
        for item in &mapping.items {
            match item {
                luma_syntax::MappingItem::Entry(entry) => {
                    self.push_mapping_entry(entry, Some(id));
                }
                luma_syntax::MappingItem::Spread(spread) => {
                    self.push_spread(spread, Some(id));
                }
                luma_syntax::MappingItem::Directive(directive) => {
                    self.push_directive(directive, Some(id));
                }
                luma_syntax::MappingItem::Conditional(block) => {
                    self.push_mapping_conditional(block, Some(id));
                }
                luma_syntax::MappingItem::Loop(block) => {
                    self.push_mapping_loop(block, Some(id));
                }
                luma_syntax::MappingItem::Let(binding) => {
                    self.push_let_binding(binding, Some(id));
                }
                luma_syntax::MappingItem::Comment(comment) => {
    ...
    fn push_sequence(
        ...
        for item in &sequence.items {
            match item {
                luma_syntax::SequenceItem::Value(node) => {
                    self.push_luma_node(node, Some(id));
                }
                luma_syntax::SequenceItem::Spread(spread) => {
                    self.push_spread(spread, Some(id));
                }
                luma_syntax::SequenceItem::Directive(directive) => {
                    self.push_directive(directive, Some(id));
                }
                luma_syntax::SequenceItem::Conditional(block) => {
                    self.push_sequence_conditional(block, Some(id));
                }
                luma_syntax::SequenceItem::Loop(block) => {
                    self.push_sequence_loop(block, Some(id));
                }
                luma_syntax::SequenceItem::Comment(comment) => {
                    self.push_comment(comment, Some(id));
                }
            }
        }
    ...
    fn push_mapping_entry(
        &mut self,
        entry: &luma_syntax::MappingEntry,
        parent: Option<usize>,
    ) -> usize {
        let id = self.push_node("map_entry", Some(entry.span), parent, Vec::new());
        self.push_mapping_key(&entry.key, Some(id));
        self.push_luma_node(&entry.value, Some(id));
        id
    }

    fn push_mapping_key(&mut self, key: &luma_syntax::MappingKey, parent: Option<usize>) -> usize {
        match key {
            luma_syntax::MappingKey::Plain { value, span, .. } => self.push_node(
                "plain_key",
                Some(*span),
                parent,
                vec![SyntaxField::new(
            ...
            luma_syntax::MappingKey::Quoted(node) => self.push_node(
                "quoted_key",
                Some(node.span),
                parent,
                vec![SyntaxField::new(
            ...
            luma_syntax::MappingKey::Expression { expression, span } => {
    ...
    fn push_tagged(&mut self, tagged: &luma_syntax::TaggedNode, parent: Option<usize>) -> usize {
        ...
        if let Some(value) = &tagged.value {
            self.push_luma_node(value, Some(id));
        }
        id
    }
    ...
    fn push_lua_expression(
        &mut self,
        kind: &str,
        expression: &luma_syntax::LuaExpression,
        parent: Option<usize>,
    ) -> usize {
        self.push_node(
            kind,
    ...
    fn push_spread(&mut self, spread: &luma_syntax::SpreadEntry, parent: Option<usize>) -> usize {
    ...
    fn push_let_binding(
        &mut self,
        binding: &luma_syntax::LetBinding,
        parent: Option<usize>,
    ) -> usize {
        ...
        self.push_luma_node(&binding.value, Some(id));
        id
    }

    fn push_comment(&mut self, comment: &luma_syntax::Comment, parent: Option<usize>) -> usize {
        self.push_node(
            "comment",
            Some(comment.span),
            parent,
    ...
    fn push_mapping_conditional(
        &mut self,
        conditional: &luma_syntax::ConditionalBlock<luma_syntax::MappingBlock>,
        parent: Option<usize>,
    ) -> usize {
    ...
    fn push_sequence_conditional(
        &mut self,
        conditional: &luma_syntax::ConditionalBlock<luma_syntax::SequenceBlock>,
        parent: Option<usize>,
    ) -> usize {
        let id = self.push_node("if_block", Some(conditional.span), parent, Vec::new());
    ...
    fn push_mapping_branch(
        &mut self,
        kind: &str,
        branch: &luma_syntax::ConditionalBranch<luma_syntax::MappingBlock>,
        parent: Option<usize>,
    ) -> usize {
    ...
    fn push_sequence_branch(
        &mut self,
        kind: &str,
        branch: &luma_syntax::ConditionalBranch<luma_syntax::SequenceBlock>,
        parent: Option<usize>,
    ) -> usize {
        let id = self.push_node(kind, Some(branch.span), parent, Vec::new());
        self.push_lua_expression("lua_expression", &branch.condition, Some(id));
    ...
    fn push_mapping_else_branch(
        &mut self,
        branch: &luma_syntax::ElseBranch<luma_syntax::MappingBlock>,
        parent: Option<usize>,
    ) -> usize {
    ...
    fn push_sequence_else_branch(
        &mut self,
        branch: &luma_syntax::ElseBranch<luma_syntax::SequenceBlock>,
        parent: Option<usize>,
    ) -> usize {
        let id = self.push_node("else_block", Some(branch.span), parent, Vec::new());
        self.push_sequence(&branch.body, Some(id));
    ...
    fn push_mapping_loop(
        &mut self,
        block: &luma_syntax::LoopBlock<luma_syntax::MappingBlock>,
        parent: Option<usize>,
    ) -> usize {
    ...
    fn push_sequence_loop(
        &mut self,
        block: &luma_syntax::LoopBlock<luma_syntax::SequenceBlock>,
        parent: Option<usize>,
    ) -> usize {
        let id = self.push_loop_header(block, parent);
    ...
    fn push_loop_header<T>(
        &mut self,
        block: &luma_syntax::LoopBlock<T>,
        parent: Option<usize>,
    ) -> usize {
        let mut fields = Vec::new();
        match &block.bindings {
            luma_syntax::LoopBindings::One { value, .. } => fields.push(SyntaxField::new(
                "binding",
                SyntaxFieldValue::TokenText(value.clone()),
            )),
            luma_syntax::LoopBindings::Two { key, value, .. } => {
                fields.push(SyntaxField::new(
                    "key",
    ...
    fn push_directive(
        &mut self,
        directive: &luma_syntax::Directive,
        parent: Option<usize>,
    ) -> usize {
        match directive {
            luma_syntax::Directive::Version(value) => self.push_node(
                "version_directive",
                Some(value.span),
                parent,
                vec![SyntaxField::new(
            ...
            luma_syntax::Directive::Profile(value) => self.push_node(
                "profile_directive",
                Some(value.span),
                parent,
                vec![SyntaxField::new(
                    ...
                    SyntaxFieldValue::TokenText(match &value.profile {
                        luma_syntax::LumaProfile::Data => "data".to_owned(),
                        luma_syntax::LumaProfile::Safe => "safe".to_owned(),
                        luma_syntax::LumaProfile::Trusted => "trusted".to_owned(),
                        luma_syntax::LumaProfile::Custom(value) => value.clone(),
                    }),
                )],
            ),
            luma_syntax::Directive::Schema(value) => {
                let id = self.push_node("schema_directive", Some(value.span), parent, Vec::new());
                self.push_luma_node(
                    &luma_syntax::LumaNode::String(value.location.clone()),
                    Some(id),
                );
                id
            }
            luma_syntax::Directive::Import(value) => {
                let id = self.push_node(
                    "import_directive",
                    Some(value.span),
                    parent,
            ...
            luma_syntax::Directive::Include(value) => {
            ...
            luma_syntax::Directive::Use(value) => self.push_node(
                "use_directive",
                Some(value.span),
                parent,
                vec![
            ...
            luma_syntax::Directive::LuaPrelude(value) => {
                let id = self.push_node(
            ...
            luma_syntax::Directive::Meta(value) => {
...
mod tests {
    ...
    fn astn_builder_uses_preorder_and_maps_spans_when_available() {
        ...
        let table = SyntaxNodeTable::from_luma_file(&parsed.file, Some(&spans));
