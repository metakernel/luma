...
/// Blob table attached to a LUMBA file.
...
impl BlobTable {
    ...
    pub fn push(&mut self, record: BlobRecord) -> Result<BlobId> {
        let blob_id = u64::try_from(self.records.len())
            .map_err(|_| LumbaError::limit_exceeded("blob count exceeds supported range"))?;
...
pub(crate) fn decode_blob_table(payload: &[u8], limits: &Limits) -> Result<BlobTable> {
    ...
    let blob_count = usize::try_from(UVar::decode(payload, &mut offset)?.0)
        .map_err(|_| LumbaError::limit_exceeded("blob count exceeds configured maximum"))?;
    ...
    let expected_offset_table_len = blob_count.checked_mul(8).ok_or_else(|| {
        LumbaError::InvalidSectionTable(ErrorContext::new("blob offset table length overflowed"))
    })?;
    ...
    let records_start = offset
        ...
        .ok_or_else(|| {
            LumbaError::InvalidSectionTable(ErrorContext::new("blob record start overflowed"))
        })?;
    ...
    for record_index in 0..blob_count {
        ...
        let byte_length = usize::try_from(UVar::decode(record, &mut record_cursor)?.0)
            .map_err(|_| LumbaError::limit_exceeded("blob length exceeds configured maximum"))?;
...
mod tests {
    ...
    use crate::{error::LumbaError, policy::Limits};
