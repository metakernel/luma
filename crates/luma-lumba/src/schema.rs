...
pub(crate) fn decode_schema_table(
    ...
    let schema_count = usize::try_from(schema_count)
        .map_err(|_| LumbaError::limit_exceeded("schema count exceeds configured maximum"))?;
    ...
    strings
        ...
        .and_then(|index| {
            u64::try_from(index)
                ...
                .ok_or_else(|| {
                    LumbaError::invalid_section_table("schema string reference overflowed u64")
                })
        })
}
    ...
    values
        ...
        .and_then(|index| {
            u64::try_from(index)
                ...
                .and_then(|index| index.checked_add(1))
                .ok_or_else(|| {
                    LumbaError::invalid_section_table("schema value reference overflowed u64")
                })
        })
}
...
fn encode_blob_ref(
    ...
    digest_blob_ref.0.checked_add(1).ok_or_else(|| {
        LumbaError::invalid_section_table("schema digest blob reference overflowed u64")
    })
}
