...
use crate::{
    blob::BlobId,
    error::{ErrorContext, LumbaError, Result},
    policy::Limits,
...
use luma_syntax::{
    ...
    LumaSequence, LumaTag, LumaTagName, LumaTaggedValue, LumaValue, source::Span,
};
...
impl FiniteFloat {
    ...
    pub fn new(value: f64) -> Result<Self> {
        if value.is_finite() {
            ...
            Err(LumbaError::unsupported_numeric_value(format!(
...
impl TryFrom<&LumaValue> for Value {
    type Error = LumbaError;

    fn try_from(value: &LumaValue) -> Result<Self> {
        match value {
            LumaValue::Null(_) => Ok(Self::Null),
            LumaValue::Boolean(value) => Ok(Self::Bool(*value)),
            LumaValue::Number(LumaNumber::Integer(value)) => Ok(Self::Int(*value)),
            LumaValue::Number(LumaNumber::Float(value)) => {
                FiniteFloat::new(*value).map(Self::Float)
            }
            LumaValue::String(value) => Ok(Self::String(value.clone())),
            LumaValue::Sequence(sequence) => sequence
                .items
                .iter()
                .map(Self::try_from)
            ...
            LumaValue::Mapping(mapping) => mapping
                ...
                .collect::<Result<Vec<_>>>()
                .map(Self::Map),
            LumaValue::Tagged(tagged) => Ok(Self::Tagged(TaggedValue {
                tag: Identifier::new(tagged.tag.name.value.clone()),
                value: Box::new(Self::try_from(tagged.value.as_ref())?),
            })),
            LumaValue::Function(host) => Err(unsupported_runtime_value("function", host)),
            LumaValue::UserData(host) => Err(unsupported_runtime_value("userdata", host)),
            LumaValue::HostObject(host) => Err(unsupported_runtime_value("host object", host)),
        }
    }
}
...
impl TryFrom<&Value> for LumaValue {
    type Error = LumbaError;

    fn try_from(value: &Value) -> Result<Self> {
        match value {
            Value::Null => Ok(Self::Null(LumaNull)),
            Value::Bool(value) => Ok(Self::Boolean(*value)),
            Value::Int(value) => Ok(Self::Number(LumaNumber::Integer(*value))),
            Value::UInt(value) => i64::try_from(*value)
                .map(|value| Self::Number(LumaNumber::Integer(value)))
                .map_err(|_| {
                    LumbaError::unsupported_numeric_value(format!(
                        "unsigned integer {value} exceeds luma_syntax::LumaValue integer range",
                    ))
                }),
            Value::Float(value) => Ok(Self::Number(LumaNumber::Float(value.get()))),
            Value::Decimal(value) => Err(LumbaError::unsupported_decimal_value(format!(
                "decimal value {:?} has no lossless luma_syntax::LumaValue representation",
                value.as_str(),
            ))),
            Value::String(value) => Ok(Self::String(value.clone())),
            Value::BytesInline(bytes) => Err(LumbaError::unsupported_byte_value(format!(
                "inline byte value of {} bytes has no lossless luma_syntax::LumaValue representation",
                bytes.len(),
            ))),
            Value::BytesBlob(blob_id) => Err(LumbaError::unsupported_byte_value(format!(
                "blob byte value {:?} has no lossless luma_syntax::LumaValue representation",
                blob_id,
            ))),
            Value::Sequence(items) => items
                ...
                .map(Self::try_from)
                .collect::<Result<Vec<_>>>()
                .map(|items| Self::Sequence(LumaSequence { items, span: None })),
            Value::Map(entries) => entries
                ...
                .collect::<Result<Vec<_>>>()
                .map(|entries| {
                    Self::Mapping(LumaMapping {
                        entries,
                        duplicate_keys: Vec::new(),
                        span: None,
                    })
                }),
            Value::Tagged(tagged) => Ok(Self::Tagged(LumaTaggedValue {
                tag: LumaTag {
                    name: LumaTagName {
            ...
            Value::ExpressionSource(expression) => {
                Err(LumbaError::unsupported_runtime_value(format!(
                    "expression source value {} is inert evaluation metadata, not a portable luma_syntax::LumaValue",
                    expression.language.as_str()
                )))
            }
            Value::LuaChunkSource(chunk) => Err(LumbaError::unsupported_runtime_value(format!(
                "chunk source value {} is inert evaluation metadata, not a portable luma_syntax::LumaValue",
                chunk.language.as_str()
            ))),
            Value::RuntimeDescriptor(descriptor) => {
                Err(LumbaError::unsupported_runtime_value(format!(
                    "runtime descriptor {} is inert host-resolution metadata, not a portable luma_syntax::LumaValue",
                    descriptor.kind.as_str()
                )))
            }
            Value::ExtensionValue(extension) => {
                Err(LumbaError::unsupported_runtime_value(format!(
                    "extension value {}:{} is inert extension metadata, not a portable luma_syntax::LumaValue",
...
pub(crate) fn decode_value_table(
    ...
    let offset_table_bytes = value_count.checked_mul(8).ok_or_else(|| {
        LumbaError::InvalidSectionTable(ErrorContext::new("value offset table length overflowed"))
    })?;
    let records_start = offset.checked_add(offset_table_bytes).ok_or_else(|| {
        LumbaError::InvalidSectionTable(ErrorContext::new("value record start overflowed"))
    })?;
    if records_start > payload.len() {
        return Err(LumbaError::InvalidSectionTable(ErrorContext::new(
...
pub(crate) fn encode_value_table(
    ...
    let table_len = builder.records.len().checked_mul(8).ok_or_else(|| {
        LumbaError::InvalidSectionTable(ErrorContext::new(
    ...
    let raw = match tag {
        ...
        RECORD_FLOAT64 => {
            ...
            if !value.is_finite() {
                let error = match mode {
                    ValueDecodeMode::Portable => LumbaError::unsupported_numeric_value(
                        "non-finite float is not supported in portable values",
                    ),
                    ValueDecodeMode::Canonical => {
                        LumbaError::non_canonical_encoding("canonical values require finite floats")
                    }
                };
                let context = error
                    .context()
        ...
        RECORD_BYTES_INLINE => {
            let len = usize::try_from(UVar::decode(record, &mut offset)?.0).map_err(|_| {
                LumbaError::limit_exceeded("blob length exceeds configured maximum")
            })?;
            let bytes =
...
fn decode_bool_flag(
    ...
    match UVar::decode(record, offset)?.0 {
        0 => Ok(false),
        1 => Ok(true),
        other => Err(LumbaError::InvalidSectionTable(
...
impl<'a> ValueArenaBuilder<'a> {
    ...
    fn push(&mut self, value: &Value, depth: usize) -> Result<usize> {
        ...
        match value {
            ...
            Value::Decimal(value) => {
                return Err(LumbaError::unsupported_decimal_value(format!(
    ...
    match value {
        Value::Map(entries) => {
            ...
            for entry in entries {
                let key = canonical_key_bytes(&entry.key);
                if !seen.insert(key) {
                    return Err(LumbaError::DuplicateKeyInCanonicalMap(ErrorContext::new(
...
fn native_key(key: &LumaKey) -> Result<Value> {
    match key {
        LumaKey::String(value) => Ok(Value::String(value.clone())),
        LumaKey::Number(LumaNumber::Integer(value)) => Ok(Value::Int(*value)),
        LumaKey::Number(LumaNumber::Float(value)) => FiniteFloat::new(*value).map(Value::Float),
        LumaKey::Boolean(value) => Ok(Value::Bool(*value)),
        LumaKey::Host(host) => Err(unsupported_runtime_value("host key", host)),
    }
}

fn syntax_key(value: &Value) -> Result<LumaKey> {
    match value {
        Value::String(value) => Ok(LumaKey::String(value.clone())),
        Value::Int(value) => Ok(LumaKey::Number(LumaNumber::Integer(*value))),
        Value::UInt(value) => i64::try_from(*value)
            .map(|value| LumaKey::Number(LumaNumber::Integer(value)))
            .map_err(|_| {
                LumbaError::unsupported_numeric_value(format!(
                    "unsigned integer key {value} exceeds luma_syntax::LumaKey integer range",
                ))
            }),
        Value::Float(value) => Ok(LumaKey::Number(LumaNumber::Float(value.get()))),
        Value::Bool(value) => Ok(LumaKey::Boolean(*value)),
        Value::Decimal(decimal) => Err(LumbaError::unsupported_decimal_value(format!(
            "decimal map key {:?} has no lossless luma_syntax::LumaKey representation",
            decimal.as_str(),
        ))),
        Value::BytesInline(bytes) => Err(LumbaError::unsupported_byte_value(format!(
        ...
        | Value::LuaChunkSource(_)
        | Value::RuntimeDescriptor(_)
        | Value::ExtensionValue(_) => Err(LumbaError::InvalidValueReference(ErrorContext::new(
    ...
    value
        .map(|value| {
            value.checked_add(1).ok_or_else(|| {
                LumbaError::invalid_section_table("optional arena reference overflowed u64")
            })
        })
        .transpose()
...
fn unsupported_runtime_value(kind: &str, host: &LumaHostValue) -> LumbaError {
    LumbaError::unsupported_runtime_value(format!(
...
mod tests {
    ...
    use luma_syntax::{
        ...
        LumaTag, LumaTagName, LumaTaggedValue, LumaValue,
    };
    ...
    #[test]
    fn vals_round_trip_from_luma_values_preserves_order_and_values() {
            ...
            LumaValue::Tagged(LumaTaggedValue {
                ...
                value: Box::new(LumaValue::Mapping(LumaMapping {
                    entries: vec![LumaMappingEntry {
                        key: LumaKey::String(String::from("k")),
    ...
    fn vals_portable_decode_rejects_non_finite_float_with_lb0024() {
        ...
        assert!(matches!(error, LumbaError::UnsupportedNumericValue(_)));
    ...
    #[test]
    fn converts_from_luma_value_for_supported_portable_values() {
        let syntax = LumaValue::Mapping(LumaMapping {
            entries: vec![
                LumaMappingEntry {
                    key: LumaKey::String(String::from("name")),
                    value: LumaValue::String(String::from("luma")),
                    span: None,
                },
                LumaMappingEntry {
                    key: LumaKey::Number(LumaNumber::Integer(7)),
                    value: LumaValue::Sequence(LumaSequence {
                        items: vec![LumaValue::Null(LumaNull), LumaValue::Boolean(true)],
                        span: None,
                    }),
                    span: None,
                },
                LumaMappingEntry {
                    key: LumaKey::Boolean(false),
                    value: LumaValue::Tagged(LumaTaggedValue {
                        tag: LumaTag {
                            name: LumaTagName {
                                value: String::from("tag"),
                                span: empty_span(),
                        ...
                        value: Box::new(LumaValue::Number(LumaNumber::Float(1.25))),
                        span: None,
                    }),
                    span: None,
                },
    ...
    #[test]
    fn converts_to_luma_value_for_supported_portable_subset() {
        ...
        let syntax = LumaValue::try_from(native).unwrap();

        assert!(matches!(syntax, LumaValue::Tagged(_)));
    }

    #[test]
    fn conversions_reject_non_portable_bytes_decimal_and_runtime_cases() {
        let byte_error = LumaValue::try_from(Value::BytesInline(vec![1, 2, 3])).unwrap_err();
        assert!(matches!(byte_error, LumbaError::UnsupportedByteValue(_)));

        let decimal_error =
            LumaValue::try_from(Value::Decimal(DecimalString::new("1.23"))).unwrap_err();
        assert!(matches!(
            decimal_error,
            LumbaError::UnsupportedDecimalValue(_)
        ));

        let runtime_error = Value::try_from(LumaValue::HostObject(LumaHostValue {
        ...
        assert!(matches!(
            runtime_error,
            LumbaError::UnsupportedRuntimeValue(_)
        ));
    }
    ...
    fn conversions_reject_non_finite_and_out_of_range_numeric_cases() {
        let non_finite =
            Value::try_from(LumaValue::Number(LumaNumber::Float(f64::INFINITY))).unwrap_err();
        assert!(matches!(non_finite, LumbaError::UnsupportedNumericValue(_)));

        let too_large = LumaValue::try_from(Value::UInt((i64::MAX as u64) + 1)).unwrap_err();
        assert!(matches!(too_large, LumbaError::UnsupportedNumericValue(_)));
    }
