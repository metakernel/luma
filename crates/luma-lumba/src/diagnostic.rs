//! Structured diagnostics for LUMBA parsing, validation, and policy enforcement.
...
/// Machine-readable diagnostic code for LUMBA failures.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum DiagnosticCode {
    /// File header magic did not match the LUMBA signature.
    InvalidMagic,
...
impl StoredDiagnosticSeverity {
    ...
    fn decode(raw: u64, record_index: usize) -> Result<Self> {
        match raw {
            ...
            3 => Ok(Self::Error),
            4 => Ok(Self::Fatal),
            _ => Err(LumbaError::InvalidSectionTable(
    ...
    #[must_use]
    pub const fn from_luma_syntax(severity: luma_syntax::Severity) -> Self {
        match severity {
            luma_syntax::Severity::Info => Self::Note,
            luma_syntax::Severity::Warning => Self::Warning,
            luma_syntax::Severity::Error => Self::Error,
        }
    }

    /// Best-effort conversion into a syntax-layer severity.
    #[must_use]
    pub const fn to_luma_syntax(self) -> luma_syntax::Severity {
        match self {
            Self::Note | Self::Help => luma_syntax::Severity::Info,
            Self::Warning => luma_syntax::Severity::Warning,
            Self::Error | Self::Fatal => luma_syntax::Severity::Error,
        }
    }
...
impl DiagnosticRecord {
    ...
    #[must_use]
    pub fn from_luma_syntax(diagnostic: &luma_syntax::Diagnostic) -> Self {
        let mut record = Self::new(
            StoredDiagnosticSeverity::from_luma_syntax(diagnostic.severity),
    ...
    #[must_use]
    pub fn from_luma_syntax_with_span_encoder<F>(
        diagnostic: &luma_syntax::Diagnostic,
        mut encode_span_ref: F,
    ) -> Self
    where
        F: FnMut(luma_syntax::Span) -> Option<u64>,
    {
        let mut record = Self::from_luma_syntax(diagnostic);
    ...
    #[must_use]
    pub fn to_luma_syntax_lossy(&self) -> Option<luma_syntax::Diagnostic> {
        let code = parse_luma_syntax_code(self.code_symbol.as_str())?;
        let mut diagnostic = luma_syntax::Diagnostic::new(code, self.severity.to_luma_syntax());
    ...
    #[must_use]
    pub fn to_luma_syntax_with_span_resolver<F>(
        &self,
        mut resolve_span_ref: F,
    ) -> Option<luma_syntax::Diagnostic>
    where
        F: FnMut(u64) -> Option<luma_syntax::Span>,
    {
        let code = parse_luma_syntax_code(self.code_symbol.as_str())?;
        let mut diagnostic = luma_syntax::Diagnostic::new(code, self.severity.to_luma_syntax());
        diagnostic.message = self.message.clone();
        ...
        diagnostic.related_spans = self
            ...
            .iter()
            .filter_map(|related| {
                Some(luma_syntax::RelatedDiagnosticSpan {
...
pub(crate) fn decode_diagnostic_table(
    ...
    for record_index in 0..diagnostic_count {
        ...
        if related_count > limits.max_table_record_count {
            return Err(LumbaError::ResourceLimitExceeded(
...
fn parse_luma_syntax_code(code: &str) -> Option<luma_syntax::DiagnosticCode> {
...
fn validate_diagnostic_flags(
    ...
    if flags & DIAGNOSTIC_FLAG_RESERVED_MASK != 0 {
        ...
        return Err(LumbaError::InvalidReservedFlags(context));
    }
