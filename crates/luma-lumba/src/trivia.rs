...
pub(crate) fn decode_trivia_table(
    ...
    let trivia_count = usize::try_from(UVar::decode(payload, &mut offset)?.0)
        .map_err(|_| LumbaError::limit_exceeded("trivia count exceeds configured maximum"))?;
    ...
    let records_offset = offset
        ...
        .ok_or_else(|| {
            LumbaError::InvalidSectionTable(ErrorContext::new("TRIV offset table end overflowed"))
        })?;
