//! Error types for `lyma-serde`.

use std::fmt;

use lyma_syntax::Diagnostic;
...
impl Error {
    ...
    /// Converts a syntax-layer diagnostic into an `lyma-serde` error.
    #[must_use]
    ...
    /// Returns `true` when the error wraps a `lyma-syntax` diagnostic.
    #[must_use]
