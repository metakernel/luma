...
use luma_core::lumba;
use luma_core::parser::FileId;
use luma_core::syntax::{
    LumaKey, LumaMapping, LumaMappingEntry, LumaNumber, LumaSequence, LumaTag, LumaTagName,
    LumaTaggedValue,
};
use luma_core::{LumaNull, LumaValue, SyntaxIndex, SyntaxNodeId};

#[pyclass(module = "luma_python")]
...
fn parse_str(py: Python<'_>, file_id: u32, name: &str, text: &str) -> PyResult<Py<PyDict>> {
    let parsed = luma_core::Parser::new().parse_str(FileId(file_id), name, text);
...
fn format_str(py: Python<'_>, file_id: u32, name: &str, text: &str) -> PyResult<Py<PyDict>> {
    let formatted = luma_core::parser::format_str(FileId(file_id), name, text);
    let out = PyDict::new_bound(py);
...
#[pyfunction]
fn from_lumba_value_image(py: Python<'_>, bytes: &[u8]) -> PyResult<PyObject> {
...
#[pyfunction]
fn read_lumba_value_image(py: Python<'_>, bytes: &[u8]) -> PyResult<PyObject> {
...
fn luma_python(py: Python<'_>, module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    let lumba_module = PyModule::new_bound(py, "lumba")?;
...
fn diagnostics_to_py(py: Python<'_>, diagnostics: &[luma_core::Diagnostic]) -> PyResult<PyObject> {
    let out = PyList::empty_bound(py);
    for diagnostic in diagnostics {
        ...
        let related = PyList::empty_bound(py);
        for related_span in &diagnostic.related_spans {
            let related_item = PyDict::new_bound(py);
...
fn syntax_index_to_py(py: Python<'_>, index: &SyntaxIndex) -> PyResult<PyObject> {
    let out = PyList::empty_bound(py);
...
fn option_span_to_py(py: Python<'_>, span: Option<luma_core::syntax::Span>) -> PyResult<PyObject> {
    match span {
...
fn span_to_py(py: Python<'_>, span: luma_core::syntax::Span) -> PyResult<PyObject> {
    let out = PyDict::new_bound(py);
...
fn py_to_luma_values(values: &Bound<'_, PyAny>) -> PyResult<Vec<LumaValue>> {
    ...
    } else if let Ok(tuple) = values.downcast::<PyTuple>() {
...
fn py_to_luma_value(obj: &Bound<'_, PyAny>) -> PyResult<LumaValue> {
    ...
    if let Ok(dict) = obj.downcast::<PyDict>() {
            ...
            (dict.get_item("__luma_tag__"), dict.get_item("value"))
        {
            ...
            let span = luma_core::syntax::Span::new(FileId(0), 0, 0);
...
fn luma_value_to_py(py: Python<'_>, value: &LumaValue) -> PyResult<PyObject> {
    match value {
        LumaValue::Null(_) => Ok(py.None()),
        LumaValue::Boolean(value) => Ok(value.into_py(py)),
        LumaValue::Number(LumaNumber::Integer(value)) => Ok(value.into_py(py)),
        LumaValue::Number(LumaNumber::Float(value)) => Ok(value.into_py(py)),
        LumaValue::String(value) => Ok(value.into_py(py)),
...
fn mapping_to_py(py: Python<'_>, mapping: &LumaMapping) -> PyResult<PyObject> {
...
fn luma_key_to_py(py: Python<'_>, key: &LumaKey) -> PyResult<PyObject> {
    match key {
        LumaKey::String(value) => Ok(value.into_py(py)),
        LumaKey::Number(LumaNumber::Integer(value)) => Ok(value.into_py(py)),
        LumaKey::Number(LumaNumber::Float(value)) => Ok(value.into_py(py)),
        LumaKey::Boolean(value) => Ok(value.into_py(py)),
        LumaKey::Host(value) => {
            let out = PyDict::new_bound(py);
