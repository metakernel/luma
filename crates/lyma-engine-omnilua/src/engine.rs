...
use crate::{
    convert::{freeze_runtime_value, to_lyma_value},
    engine_name,
...
pub struct OmniLuaChunk {
    ...
    pub(crate) span: Option<lyma_syntax::source::Span>,
}

/// Lyma runtime engine backed by `omnilua`.
...
impl RuntimeValueCodec for OmniLuaEngine {
    ...
    fn to_lyma_value(
        ...
        to_lyma_value(value, policy, self.active_max_table_entries())
    }
    ...
    fn freeze_value(&self, value: &Self::Value) -> Result<Self::FrozenValue, LuaRuntimeError> {
        match value {
            ...
            OmniLuaValue::Live(value) => to_lyma_value(
