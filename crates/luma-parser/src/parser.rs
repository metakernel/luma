...
impl Parser {
    ...
    fn parse_file(&mut self) -> LumaFile {
    ...
    fn parse_node(&mut self, index: usize, indent: usize) -> (LumaNode, usize) {
        let code = self.code(index).to_owned();
    ...
    fn parse_mapping(&mut self, mut index: usize, indent: usize) -> (LumaNode, usize) {
        ...
        while index < self.lines.len() {
            ...
            if !canonical.is_empty() {
                if let Some((first_index, first_span)) = seen.get(&canonical).copied() {
                    ...
                    duplicates.push(luma_syntax::DuplicateKey {
                        key: canonical.clone(),
                        first_index,
    ...
    #[allow(clippy::too_many_lines)]
    fn parse_sequence(&mut self, mut index: usize, indent: usize) -> (LumaNode, usize) {
        let start = self.lines[index].start + indent;
        let mut items = Vec::new();
        let mut last_end = start;
        while index < self.lines.len() {
        ...
        (
            LumaNode::Sequence(SequenceBlock {
                items,
                span: Span::new(self.file_id(), start, last_end),
            }),
            index,
    ...
    fn parse_sequence_mapping_item(
        ...
        if child < self.lines.len() && self.lines[child].indent > indent {
            ...
            if let LumaNode::Mapping(mapping) = node {
    ...
    fn parse_lua_prelude(&mut self, index: usize) -> (Directive, usize) {
        ...
        let LumaNode::LuaChunk(block) = node else {
            unreachable!()
        };
        (
            make_lua_prelude(
    ...
    fn parse_meta(&mut self, index: usize) -> (Directive, usize) {
        ...
        let LumaNode::Mapping(value) = node else {
            unreachable!()
        };
        ...
        while next < self.lines.len() && self.lines[next].indent == indent {
            ...
            match parse_directive_line(
                ...
                Some(DirectiveParse::Else) => {
                    ...
                    {
                        let (node, consumed) = self.parse_mapping(child, self.lines[child].indent);
                        let LumaNode::Mapping(body) = node else {
                            unreachable!()
                        };
                        (body, consumed)
                    } else {
            ...
            match parse_directive_line(
                ...
                Some(DirectiveParse::Else) => {
                        ...
                        && self.lines[child].indent > indent
                    {
                        ...
                        let LumaNode::Sequence(body) = node else {
                            unreachable!()
                        };
                        (body, consumed)
                    } else {
    ...
    fn parse_nested_block(&mut self, index: usize, parent_indent: usize) -> (LumaNode, usize) {
...
mod tests {
    ...
    fn parses_mapping_sequence_and_scalars() {
        ...
        let DocumentItem::Root(LumaNode::Mapping(mapping)) = &document.items[0] else {
            panic!()
        };
        ...
        let LumaNode::String(value) = &name.value else {
            panic!()
        };
    ...
    fn rejects_duplicate_keys() {
        let parsed = parse_str(FileId(1), "dup.luma", "name: one\nname: two\n");
        assert!(
            parsed
                .diagnostics
                .iter()
