...
use luma_syntax::{FileId, TextEdit, TextRange, apply_text_edits};
...
#[cfg(test)]
mod tests {
    use luma_syntax::{DiagnosticCode, FileId, LumaNode, MappingItem, TextRange};
    ...
    fn session_parses_initial_text_and_reports_full_reparse_metadata() {
        let mut session = ParseSession::new(FileId(1), "example.luma");
    ...
    fn session_applies_insert_replace_and_delete_changes() {
        ...
        let luma_syntax::DocumentItem::Root(LumaNode::Mapping(mapping)) = &document.items[0] else {
            panic!();
        };
    ...
    fn session_preserves_diagnostic_and_span_updates_after_incremental_change() {
        ...
        let luma_syntax::MappingKey::Plain { span, .. } = &entry.key else {
            panic!();
        };
