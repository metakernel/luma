...
use luma_syntax::{Diagnostic, DiagnosticCode, FileId, LumaSource, Span};
...
/// Decodes raw bytes as normalized UTF-8 Luma source.
///
/// # Errors
///
...
pub fn decode_str(
    ...
    Ok(SourceText {
        source: LumaSource::new(file_id, name, normalized),
    })
}
...
mod tests {
    ...
    fn removes_bom_and_normalizes_crlf() {
        let source = decode_str(FileId(1), "spec.luma", "\u{feff}a\r\n b\r\n").unwrap();
        assert_eq!(source.as_str(), "a\n b\n");
    }

    #[test]
    fn rejects_invalid_utf8() {
        let error = decode_bytes(FileId(1), "spec.luma", &[0xf0, 0x28, 0x8c, 0x28]).unwrap_err();
    ...
    fn rejects_nul_bytes() {
        let error = decode_str(FileId(1), "spec.luma", "a\0b").unwrap_err();
