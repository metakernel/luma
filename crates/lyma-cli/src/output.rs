use clap::ValueEnum;
use lyma_parser::{Diagnostic, LymaFile, LymaNode, LymaSource, Severity};
use lyma_syntax::{
    ConditionalBlock, ConditionalBranch, Directive, Document, DocumentItem, LoopBindings,
    LoopBlock, LuaExpression, LymaProfile, MappingBlock, MappingItem, SequenceBlock, SequenceItem,
    Span,
};
#[cfg(feature = "eval")]
use lyma_syntax::{LymaHostValue, LymaKey, LymaValue};
...
impl DiagnosticFactory {
    pub fn engine_unavailable() -> Diagnostic {
        let mut diagnostic = Diagnostic::new(
            lyma_parser::DiagnosticCode::UnsafeOperation,
            Severity::Error,
        );
        diagnostic.message =
            "eval requires the lyma-cli 'engine-omnilua' feature or an enabled evaluation backend"
                .to_owned();
        diagnostic
    }
}
...
pub fn diagnostic_to_json(diagnostic: &Diagnostic, source: Option<&LymaSource>) -> Value {
    json!({
...
pub fn ast_to_json(file: &LymaFile) -> Value {
    json!({
        "type": "file",
...
#[cfg(feature = "eval")]
pub fn evaluated_document_to_json(document: &lyma_eval::EvaluatedDocument) -> Value {
    json!({
...
fn node_to_json(node: &LymaNode) -> Value {
    match node {
        LymaNode::Null { span } => json!({"type":"null","span": span_to_json(*span, None)}),
        LymaNode::Boolean { value, span } => {
        ...
        LymaNode::String(string) => string_node_to_json(string),
        LymaNode::Sequence(sequence) => sequence_to_json(sequence),
        LymaNode::Mapping(mapping) => mapping_to_json(mapping),
...
fn string_node_to_json(string: &lyma_parser::StringNode) -> Value {
    json!({
        "type": "string",
        "value": string.value,
...
fn mapping_key_to_json(key: &lyma_parser::MappingKey) -> Value {
    match key {
        lyma_parser::MappingKey::Plain { value, span, .. } => {
            json!({"type":"plain","value":value,"span": span_to_json(*span, None)})
        }
        lyma_parser::MappingKey::Quoted(value) => {
            json!({"type":"quoted","value": string_node_to_json(value)})
        }
        lyma_parser::MappingKey::Expression { expression, span } => {
...
fn profile_to_json(profile: &LymaProfile) -> Value {
    match profile {
        LymaProfile::Data => Value::String("data".to_owned()),
        LymaProfile::Safe => Value::String("safe".to_owned()),
        LymaProfile::Trusted => Value::String("trusted".to_owned()),
        LymaProfile::Custom(value) => Value::String(value.clone()),
    }
}

#[cfg(feature = "eval")]
pub fn value_to_json(value: &LymaValue) -> Value {
    match value {
        LymaValue::Null(_) => Value::Null,
        LymaValue::Boolean(value) => Value::Bool(*value),
        LymaValue::Number(number) => match number {
            lyma_syntax::LymaNumber::Integer(value) => json!({"type":"integer","value": value}),
            lyma_syntax::LymaNumber::Float(value) => json!({"type":"float","value": value}),
        },
        LymaValue::String(value) => Value::String(value.clone()),
        ...
        LymaValue::Function(value) => host_value_json("function", value),
        LymaValue::UserData(value) => host_value_json("userdata", value),
        LymaValue::HostObject(value) => host_value_json("host_object", value),
    }
}

#[cfg(feature = "eval")]
fn key_to_json(key: &LymaKey) -> Value {
    match key {
        LymaKey::String(value) => json!({"type":"string","value": value}),
        LymaKey::Number(number) => {
            json!({"type":"number","value": value_to_json(&LymaValue::Number(number.clone()))})
        }
        LymaKey::Boolean(value) => json!({"type":"boolean","value": value}),
        LymaKey::Host(value) => host_value_json("host", value),
    }
}

#[cfg(feature = "eval")]
fn host_value_json(kind: &str, value: &LymaHostValue) -> Value {
    json!({"type": kind, "kind": value.kind, "label": value.label})
}

pub fn span_to_json(span: Span, source: Option<&LymaSource>) -> Value {
