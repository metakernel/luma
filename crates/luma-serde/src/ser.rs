...
/// Serde serializer adapter that produces Luma values.
...
impl ValueSerializer {
    ...
    fn serialize_signed(value: i128) -> Result<LumaValue> {
        let value = i64::try_from(value)
            .map_err(|_| Self::unsupported_named("integer outside Luma i64 range"))?;
        Ok(LumaValue::Number(LumaNumber::Integer(value)))
    }

    fn serialize_unsigned(value: u128) -> Result<LumaValue> {
        let value = i64::try_from(value)
            .map_err(|_| Self::unsupported_named("integer outside Luma i64 range"))?;
        Ok(LumaValue::Number(LumaNumber::Integer(value)))
    }
...
impl MapSerializer {
    ...
    fn finish(self) -> Result<LumaValue> {
...
fn wrap_variant(variant: &'static str, value: LumaValue) -> LumaValue {
    LumaValue::Mapping(LumaMapping {
        entries: vec![LumaMappingEntry {
            key: LumaKey::String(variant.to_owned()),
            value,
            span: None,
        }],
        duplicate_keys: Vec::new(),
...
fn value_to_key(value: LumaValue) -> Result<LumaKey> {
    match value {
...
fn ensure_portable_mapping_keys(value: &LumaValue) -> Result<()> {
    match value {
        LumaValue::Mapping(mapping) => {
            for entry in &mapping.entries {
                match &entry.key {
                    LumaKey::String(_) => {}
                    LumaKey::Number(_) | LumaKey::Boolean(_) => {
        ...
        LumaValue::Tagged(tagged) => ensure_portable_mapping_keys(&tagged.value),
        LumaValue::Null(_)
        | LumaValue::Boolean(_)
        | LumaValue::Number(_)
        | LumaValue::String(_)
        ...
        | LumaValue::HostObject(_) => Ok(()),
    }
}

/// Converts a Serde-serializable Rust value into a Luma value.
///
/// # Errors
///
/// Returns an error when the input contains unsupported Serde data, including
/// map keys that do not serialize to scalar Luma keys.
pub fn to_value<T>(value: T) -> Result<LumaValue>
where
    T: Serialize,
{
...
/// Converts a Serde-serializable Rust value into canonical Luma text.
///
/// # Errors
///
/// Returns an error if conversion to a Luma value fails or if the resulting value
/// cannot be rendered as portable Luma text. Unlike [`to_value`], this requires
...
/// Converts a Serde-serializable Rust value into canonical Luma text with explicit options.
///
/// # Errors
///
/// Returns an error if conversion to a Luma value fails or if the resulting value
...
impl serde::Serializer for ValueSerializer {
    ...
    fn serialize_char(self, value: char) -> Result<Self::Ok> {
        Ok(LumaValue::String(value.to_string()))
    }

    fn serialize_str(self, value: &str) -> Result<Self::Ok> {
        Ok(LumaValue::String(value.to_owned()))
    }
    ...
    fn serialize_unit_variant(
        ...
        Ok(LumaValue::String(variant.to_owned()))
    }
...
impl SerializeMap for MapSerializer {
        ...
        T: ?Sized + Serialize,
    {
        ...
        self.entries.push(LumaMappingEntry {
            key,
...
impl SerializeStruct for MapSerializer {
    ...
    {
        self.entries.push(LumaMappingEntry {
            key: LumaKey::String(key.to_owned()),
            value: value.serialize(ValueSerializer::new())?,
            span: None,
        });
        Ok(())
...
mod tests {
    ...
    use luma_syntax::{
        LumaKey, LumaMapping, LumaNull, LumaNumber, SerializeOptions, serialize_value,
    ...
    fn serializes_primitives_and_structures_in_order() {
        ...
        assert!(matches!(
            value,
            luma_syntax::LumaValue::Sequence(luma_syntax::LumaSequence { items, .. })
            if matches!(items[0], luma_syntax::LumaValue::String(ref s) if s == "hello")
                && matches!(items[1], luma_syntax::LumaValue::Number(luma_syntax::LumaNumber::Integer(5)))
                && matches!(items[2], luma_syntax::LumaValue::Null(_))
                && matches!(items[3], luma_syntax::LumaValue::Null(_))
                && matches!(items[4], luma_syntax::LumaValue::Mapping(_))
        ));
    }
    ...
    fn serializes_primitive_conversions_exactly() {
        ...
        assert_eq!(
            ...
            luma_syntax::LumaValue::Number(luma_syntax::LumaNumber::Integer(i64::MAX))
        );
        assert_eq!(
            to_value(1.25_f32).unwrap(),
            luma_syntax::LumaValue::Number(luma_syntax::LumaNumber::Float(1.25))
        );
        assert_eq!(
            to_value(-2.5_f64).unwrap(),
            luma_syntax::LumaValue::Number(luma_syntax::LumaNumber::Float(-2.5))
        );
    }
    ...
    fn serializes_structs_newtypes_and_enum_variants() {
        assert!(matches!(
            ...
            .unwrap(),
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 2
                && matches!(&entries[0].key, luma_syntax::LumaKey::String(key) if key == "name")
                && matches!(&entries[1].key, luma_syntax::LumaKey::String(key) if key == "enabled")
        ));

        assert_eq!(
            to_value(Newtype(7)).unwrap(),
            luma_syntax::LumaValue::Number(luma_syntax::LumaNumber::Integer(7))
        );

        assert_eq!(
            to_value(ExampleEnum::Unit).unwrap(),
            luma_syntax::LumaValue::String(String::from("Unit"))
        );
        ...
        assert!(matches!(
            ...
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 1
                && matches!(&entries[0].key, luma_syntax::LumaKey::String(key) if key == "Tuple")
                && matches!(entries[0].value, luma_syntax::LumaValue::Sequence(_))
        ));
        ...
        assert!(matches!(
            ...
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 1
                && matches!(entries[0].value, luma_syntax::LumaValue::Mapping(_))
        ));
        ...
        assert!(matches!(
            ...
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 1
                && matches!(entries[0].value, luma_syntax::LumaValue::Boolean(true))
        ));
    }
    ...
    fn maps_none_and_unit_to_null_and_rejects_unsupported_values() {
        ...
        assert_eq!(
            int_error,
            Error::Unsupported {
                operation: "integer outside Luma i64 range"
            }
        );
    ...
    fn string_key_maps_serialize_to_value_and_text() {
        ...
        assert!(matches!(
            ...
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 2
                && matches!(&entries[0].key, LumaKey::String(key) if key == "alpha")
                && matches!(&entries[1].key, LumaKey::String(key) if key == "beta")
        ));
    ...
    #[test]
    fn to_string_matches_luma_syntax_canonical_output_for_equivalent_value() {
    ...
    #[test]
    fn to_string_with_options_matches_luma_syntax_output_for_equivalent_value() {
    ...
    fn to_value_accepts_numeric_and_boolean_map_keys() {
        ...
        assert!(matches!(
            ...
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 1
                && matches!(&entries[0].key, LumaKey::Number(LumaNumber::Integer(7)))
                && matches!(&entries[0].value, luma_syntax::LumaValue::String(value) if value == "seven")
        ));
        ...
        assert!(matches!(
            ...
            luma_syntax::LumaValue::Mapping(LumaMapping { entries, .. })
            if entries.len() == 1
                && matches!(&entries[0].key, LumaKey::Boolean(true))
                && matches!(entries[0].value, luma_syntax::LumaValue::Number(LumaNumber::Integer(1)))
        ));
    }
