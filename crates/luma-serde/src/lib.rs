//! Serde adapters for converting between Rust data structures and Luma values.
//!
...
//! Luma data:
//!
//! - booleans, strings, and `char` as scalar Luma booleans/strings (`char`
//!   serializes as a single-character string)
//! - signed and unsigned integers that fit in Luma's `i64` range
...
//! - `u64`/`u128` and `i128` values outside the Luma `i64` range => rejected as
//!   unsupported `"integer outside Luma i64 range"`
//! - map keys must serialize to scalar Luma keys only: strings, numbers, or
...
//! result through `luma_syntax`'s canonical text serializer. They are stricter
...
//! `from_value` accepts:
//!
//! - scalar Luma null/bool/number/string values for matching scalar Serde targets
...
//! - strings, single-entry mappings, or tagged Luma values for enums
//!
//! Tagged Luma values are transparent for non-enum targets: the tag wrapper is
...
//!
//! Deserialization fails when the requested Serde shape does not match the Luma
...
//! - runtime-only Luma values or host mapping keys fail with explicit errors such
//!   as `"cannot deserialize runtime-only Luma function value ..."`
