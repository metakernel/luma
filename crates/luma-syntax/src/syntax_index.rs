//! Non-owning syntax index for parsed Luma trees.

#![allow(missing_docs)]

use crate::ast::{
    Comment, ConditionalBlock, ConditionalBranch, Directive, Document, DocumentItem, ElseBranch,
    LetBinding, LoopBlock, LuaExpression, LumaFile, LumaNode, MappingBlock, MappingEntry,
...
/// Non-owning index over a parsed [`LumaFile`].
///
...
impl From<&LumaFile> for SyntaxIndex {
    fn from(value: &LumaFile) -> Self {
        Self::new(value)
    }
}
...
impl SyntaxIndexBuilder {
    fn build(mut self, file: &LumaFile) -> SyntaxIndex {
        self.push_file(file);
        SyntaxIndex {
    ...
    fn push_file(&mut self, file: &LumaFile) {
    ...
    fn push_document_item(&mut self, item: &DocumentItem, parent: SyntaxNodeId) {
        match item {
            ...
            DocumentItem::Root(node) => self.push_luma_node(node, parent),
            DocumentItem::Comment(comment) => self.push_comment(comment, parent),
        }
    }

    fn push_luma_node(&mut self, node: &LumaNode, parent: SyntaxNodeId) {
        match node {
            LumaNode::Null { span } => {
                self.push(SyntaxKind::Null, *span, Some(parent));
            }
            LumaNode::Boolean { span, .. } => {
            ...
            LumaNode::Sequence(sequence) => self.push_sequence_block(sequence, parent),
            LumaNode::Mapping(mapping) => self.push_mapping_block(mapping, parent),
            LumaNode::Tagged(tagged) => self.push_tagged_node(tagged, parent),
    ...
    fn push_sequence_item(&mut self, item: &SequenceItem, parent: SyntaxNodeId) {
        match item {
            SequenceItem::Value(node) => self.push_luma_node(node, parent),
    ...
    fn push_mapping_entry(&mut self, entry: &MappingEntry, parent: SyntaxNodeId) {
        ...
        self.push_luma_node(&entry.value, id);
    }
    ...
    fn push_let_binding(&mut self, binding: &LetBinding, parent: SyntaxNodeId) {
        ...
        self.push_luma_node(&binding.value, id);
    }
