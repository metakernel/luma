...
use lyma_syntax::{Diagnostic, DiagnosticCode, LymaTag, LymaValue, Severity};
...
pub trait TagResolver {
    ...
    ) -> Result<LymaValue, TagResolutionError>;
}
...
type TagHandler = Arc<dyn Fn(&LymaValue) -> Result<LymaValue, TagResolutionError> + Send + Sync>;
...
impl InMemoryTagResolver {
    ...
    where
        F: Fn(&LymaValue) -> Result<LymaValue, TagResolutionError> + Send + Sync + 'static,
    {
...
#[cfg(test)]
mod tests {
    use lyma_syntax::{FileId, LymaTagName, Span};
    ...
    fn deny_all_tag_resolver_rejects_tag_construction() {
        ...
        let error = DenyAllTagResolver
            .resolve_tag(TagResolutionRequest {
                tag: &tag,
                value: &lyma_syntax::LymaValue::String(String::from("x")),
            })
    ...
    fn in_memory_tag_resolver_runs_registered_handlers() {
        ...
        let resolver = InMemoryTagResolver::new().with_handler("upper", |value| match value {
            lyma_syntax::LymaValue::String(text) => {
                Ok(lyma_syntax::LymaValue::String(text.to_uppercase()))
            }
            _ => Err(super::TagResolutionError::new(
                lyma_syntax::DiagnosticCode::InvalidTagResolverResult,
        ...
        let result = resolver
            .resolve_tag(TagResolutionRequest {
                tag: &tag,
                value: &lyma_syntax::LymaValue::String(String::from("hello")),
            })
            .expect("registered tag handler should run");

        assert_eq!(
            result,
            lyma_syntax::LymaValue::String(String::from("HELLO"))
        );
    }
}
