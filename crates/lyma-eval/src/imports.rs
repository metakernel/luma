...
use lyma_syntax::{DiagnosticCode, FileId, LymaFile};
...
/// Resolves and parses a Lyma resource.
///
/// # Errors
///
...
pub fn load_lyma_resource(
    resolver: &dyn ResourceResolver,
    kind: ResolutionKind,
    specifier: &str,
    from: Option<&ResourceLocator>,
    context: &mut ResolutionContext,
) -> Result<(ResourceLocator, LymaFile, String), EvaluationError> {
    ...
    if let Some(diagnostic) = parsed
        .diagnostics
        .into_iter()
        .find(|d| d.severity == lyma_syntax::Severity::Error)
    {
    ...
    if parsed.file.documents.is_empty() {
        return Err(EvaluationError::new(
            ...
            "resolved Lyma resource did not contain a document",
        ));
    }
    Ok((
