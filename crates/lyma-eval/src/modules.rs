...
pub trait ModuleRegistry<E: LuaRuntimeEngine> {
    ...
    ) -> Result<<E as lyma_runtime::RuntimeModuleFactory>::Module, ModuleLookupError>;
}
...
pub struct InMemoryModuleRegistry {
    ...
    modules: BTreeMap<String, Vec<(String, LymaValue)>>,
}
...
impl<E: LuaRuntimeEngine> ModuleRegistry<E> for InMemoryModuleRegistry {
    fn lookup_module(
        ...
        for (name, value) in exports {
            let runtime_value = engine.from_lyma_value(value).map_err(|error| {
                ModuleLookupError::new(
...
mod tests {
    ...
    fn in_memory_registry_rejects_traversal_with_stable_diagnostic() {
        ...
        let _ = Span::new(lyma_syntax::FileId(0), 0, 0);
    }
}
