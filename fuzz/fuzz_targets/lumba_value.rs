...
use luma_lumba::section::{CHECKSUM_NONE, CODEC_NONE, SectionEntry, SectionId};
...
fuzz_target!(|data: &[u8]| {
    ...
    assert!(first.is_ok(), "lumba_value panicked on {} bytes", data.len());

    let second = std::panic::catch_unwind(|| decode_once(data));
    assert!(second.is_ok(), "lumba_value panicked on deterministic replay");
