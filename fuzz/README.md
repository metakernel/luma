...
Targets:
- `lyba_read`: raw container bytes for header/table/section decoding.
- `lyba_value`: raw `VALS` payload bytes wrapped into a bounded one-section container before decode.
...
```powershell
cargo +nightly fuzz run lyba_read fuzz/corpus/lyba_read -- -max_total_time=30 -rss_limit_mb=256 -timeout=5
cargo +nightly fuzz run lyba_value fuzz/corpus/lyba_value -- -max_total_time=30 -rss_limit_mb=256 -timeout=5
```
...
```powershell
cargo +nightly fuzz check lyba_read
cargo +nightly fuzz check lyba_value
cargo test -p lyma-lyba --test properties
```
